...
bool & m_breakOnRealloc(void *reportedAddress)
{
    ...
    m_assert(au->allocationType == m_alloc_malloc ||
             au->allocationType == m_alloc_calloc || au->allocationType == m_alloc_realloc);
                  ...
                  const unsigned int allocationType, const size_t reportedSize)
{
    try {
        ...
        if (!reservoir) {
            ...
            sAllocUnit **temp =
                (sAllocUnit **) realloc(reservoirBuffer, (reservoirBufferSize + 1) * sizeof(sAllocUnit *));
            m_assert(temp);
            if (temp) {
    ...
    try {
        ...
        if (alwaysLogAll)
            log("[-] ----- %8s of addr 0x%08X           by %s", allocationTypes[deallocationType],
                (unsigned int) reportedAddress, ownerString(sourceFile, sourceLine, sourceFunc));
