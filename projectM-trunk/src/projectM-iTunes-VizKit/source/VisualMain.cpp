...
OSStatus VisualMain::VisualPluginHandler(OSType message, VisualPluginMessageInfo * visualPluginMessageInfo,
                                         void *refCon)
{
    ...
    if (debug == true) {
        ...
        if (message != messageWriteLogHistory[curr]) {
            ...
            if (message == messageWriteLogHistory[prev] && prevMessage == messageWriteLogHistory[curr])
                doWriteLog = false;
            if (message == messageWriteLogHistory[oneBeforePrev] && prevMessage == messageWriteLogHistory[curr]
                && oneBeforePrevMessage == messageWriteLogHistory[prev])
                doWriteLog = false;
            if (message == messageWriteLogHistory[prev] && prevMessage == messageWriteLogHistory[oneBeforePrev]
                && oneBeforePrevMessage == messageWriteLogHistory[curr])
                doWriteLog = false;
            if (doWriteLog == true) {
    ...
    case kVisualPluginInitMessage:
        ...
        VisualMainAction::setAppVersionNum(visualPluginMessageInfo->u.initMessage.appVersion.majorRev,
                                           visualPluginMessageInfo->u.initMessage.appVersion.minorAndBugRev);

        VisualMainAction::handleInitMessage(kVisualMaxDataChannels, kVisualNumWaveformEntries,
    ...
    case kVisualPluginShowWindowMessage:
...

        {
            ...
            VisualMainAction::setTotalVisualizerRectVerticalOffsetFromBottom(visualPluginMessageInfo->u.
                                                                             showWindowMessage.totalVisualizerRect.top,
                                                                             visualPluginMessageInfo->u.
                                                                             showWindowMessage.totalVisualizerRect.
                                                                             bottom -
                                                                             visualPluginMessageInfo->u.
                                                                             showWindowMessage.totalVisualizerRect.top);
            VisualMainAction::setTotalVisualizerRect(visualPluginMessageInfo->u.showWindowMessage.totalVisualizerRect.
                                                     top,
                                                     visualPluginMessageInfo->u.showWindowMessage.totalVisualizerRect.
                                                     left,
                                                     visualPluginMessageInfo->u.showWindowMessage.totalVisualizerRect.
                                                     bottom,
                                                     visualPluginMessageInfo->u.showWindowMessage.totalVisualizerRect.
                                                     right);
            VisualMainAction::setDrawRect(visualPluginMessageInfo->u.showWindowMessage.drawRect.top,
                                          visualPluginMessageInfo->u.showWindowMessage.drawRect.left,
                                          visualPluginMessageInfo->u.showWindowMessage.drawRect.bottom,
                                          visualPluginMessageInfo->u.showWindowMessage.drawRect.right);
    ...
    case kVisualPluginSetWindowMessage:
        ...
        {
            VisualMainAction::setTotalVisualizerRectVerticalOffsetFromBottom(visualPluginMessageInfo->u.
                                                                             setWindowMessage.totalVisualizerRect.top,
                                                                             visualPluginMessageInfo->u.
                                                                             setWindowMessage.totalVisualizerRect.
                                                                             bottom -
                                                                             visualPluginMessageInfo->u.
                                                                             setWindowMessage.totalVisualizerRect.top);
            VisualMainAction::setTotalVisualizerRect(visualPluginMessageInfo->u.setWindowMessage.totalVisualizerRect.
                                                     top,
                                                     visualPluginMessageInfo->u.setWindowMessage.totalVisualizerRect.
                                                     left,
                                                     visualPluginMessageInfo->u.setWindowMessage.totalVisualizerRect.
                                                     bottom,
                                                     visualPluginMessageInfo->u.setWindowMessage.totalVisualizerRect.
                                                     right);
            VisualMainAction::setDrawRect(visualPluginMessageInfo->u.setWindowMessage.drawRect.top,
                                          visualPluginMessageInfo->u.setWindowMessage.drawRect.left,
                                          visualPluginMessageInfo->u.setWindowMessage.drawRect.bottom,
                                          visualPluginMessageInfo->u.setWindowMessage.drawRect.right);
    ...
    case kVisualPluginRenderMessage:
        ...
        VisualMainAction::handleRenderMessage(visualPluginMessageInfo->u.renderMessage.currentPositionInMS,
                                              visualPluginMessageInfo->u.renderMessage.timeStampID,
                                              visualPluginMessageInfo->u.renderMessage.renderData->numWaveformChannels,
                                              kVisualNumWaveformEntries,
                                              (char *) visualPluginMessageInfo->u.renderMessage.renderData->
                                              waveformData,
                                              visualPluginMessageInfo->u.renderMessage.renderData->numSpectrumChannels,
                                              kVisualNumSpectrumEntries,
                                              (char *) visualPluginMessageInfo->u.renderMessage.renderData->
                                              spectrumData);
    ...
    case kVisualPluginPlayMessage:
...

        {
            ...
            if (isAudioStream) {
                if (visualPluginMessageInfo->u.playMessage.streamInfoUnicode) {
                    VisualMain::storeAudioStreamMetadata(const_cast <
                                                         const ITStreamInfo *
                                                         >(visualPluginMessageInfo->u.playMessage.streamInfoUnicode));
                } else if (visualPluginMessageInfo->u.playMessage.streamInfo) {
                    VisualMain::storeAudioStreamMetadata(const_cast <
                                                         const ITStreamInfoV1 *
                                                         >(visualPluginMessageInfo->u.playMessage.streamInfo));
                }
            }
            bool trackDidChange =
                VisualMain::storeAudioTrackMetadata(visualPluginMessageInfo->u.playMessage.trackInfoUnicode,
                                                    isAudioStream);
    ...
    case kVisualPluginChangeTrackMessage:
...

        {
            ...
            if (visualPluginMessageInfo->u.changeTrackMessage.streamInfoUnicode) {
                isAudioStream =
                    (bool) (visualPluginMessageInfo->u.changeTrackMessage.streamInfoUnicode->streamTitle[0] > 0
            ...
            if (isAudioStream) {
                if (visualPluginMessageInfo->u.changeTrackMessage.streamInfoUnicode) {
                    VisualMain::storeAudioStreamMetadata(const_cast <
                                                         const ITStreamInfo *
                                                         >(visualPluginMessageInfo->u.changeTrackMessage.
                                                           streamInfoUnicode));
                } else if (visualPluginMessageInfo->u.changeTrackMessage.streamInfo) {
                    VisualMain::storeAudioStreamMetadata(const_cast <
                                                         const ITStreamInfoV1 *
                                                         >(visualPluginMessageInfo->u.changeTrackMessage.streamInfo));
                }
            }
            bool trackDidChange =
                VisualMain::storeAudioTrackMetadata(visualPluginMessageInfo->u.changeTrackMessage.trackInfoUnicode,
                                                    isAudioStream);
...
OSStatus VisualMain::RegisterVisualPlugin(PluginMessageInfo * pluginMessageInfo)
{
    ...
    playerMessageInfo.u.registerVisualPluginMessage.options =
        (kVisualWantsConfigure | kVisualWantsIdleMessages | kVisualProvidesUnicodeName);

    playerMessageInfo.u.registerVisualPluginMessage.name[0] = (UInt8) strlen(VisualConfiguration::visualizerShowName);
    memcpy(&playerMessageInfo.u.registerVisualPluginMessage.name[1], VisualConfiguration::visualizerShowName,
           strlen(VisualConfiguration::visualizerShowName));

    uint32 numberOfCharactersOfVisualizerShowNameUnicode = 0;
    uint16 *visualizerShowNameUnicode =
        VisualConfiguration::getVisualizerShowNameUnicode(numberOfCharactersOfVisualizerShowNameUnicode);
    ...
    SetNumVersion(&playerMessageInfo.u.registerVisualPluginMessage.pluginVersion,
                  VisualConfiguration::getMajorReleaseNumberAsBCD(), VisualConfiguration::getMinorReleaseNumberAsBCD(),
                  stage, VisualConfiguration::getReleaseRevisionNumber());
    ...
    OSStatus status =
        PlayerRegisterVisualPlugin(pluginMessageInfo->u.initMessage.appCookie, pluginMessageInfo->u.initMessage.appProc,
                                   &playerMessageInfo);
