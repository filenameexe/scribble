...
void OptionsDialog::show()
{
    ...
    err =
        GetControlByID(theOptionsDialog->optionsDialogWindow, &kCheckForUpdateCheckboxControlID,
                       &checkForUpdateControl);
    if ((err != noErr) || (!checkForUpdateControl)) {
        char errLog[256];
        sprintf(errLog, "GetControlByID returned no checkForUpdateControl (err: %ld) in file: %s (line: %d) [%s])", err,
    ...
    if ((err != noErr) || (!tabControl)) {
        char errLog[256];
        sprintf(errLog, "GetControlByID returned no tabControl (err: %ld) in file: %s (line: %d) [%s])", err, __FILE__,
...
void OptionsDialog::handleEvent(EventHandlerCallRef inRef, EventRef inEvent, void *userData)
{
    ...
    case kEventClassWindow:
        ...
        if (eventKind == kEventWindowClose) {
            ...
            if (status != noErr) {
                char errLog[256];
                sprintf(errLog, "SetWindowAlpha failed (status: %ld) in file: %s (line: %d) [%s])", status, __FILE__,
                        __LINE__, __FUNCTION__);
                writeLog(errLog);
            }
    ...
    case kEventClassControl:
        {
            ...
            GetEventParameter(inEvent, kEventParamDirectObject, typeControlRef, NULL, sizeof(ControlRef), NULL,
                              &control);
...
bool OptionsDialog::loadWindowFromResource()
{
    ...
    if (!thePlugin) {
        char errLog[256];
        sprintf(errLog, "CFBundleGetBundleWithIdentifier returned NULL in file: %s (line: %d) [%s])", __FILE__,
                __LINE__, __FUNCTION__);
        writeLog(errLog);
        return success;
    }
    ...
    if (err != noErr) {
        char errLog[256];
        sprintf(errLog, "CreateNibReferenceWithCFBundle failed (err: %ld) in file: %s (line: %d) [%s])", err, __FILE__,
                __LINE__, __FUNCTION__);
        writeLog(errLog);
        return success;
    }
    ...
    if (err != noErr) {
        char errLog[256];
        sprintf(errLog, "CreateWindowFromNib failed (err: %ld) in file: %s (line: %d) [%s])", err, __FILE__, __LINE__,
                __FUNCTION__);
        writeLog(errLog);
        return success;
    }
    ...
    if (err != noErr) {
        char errLog[256];
        sprintf(errLog, "SetWindowAlpha failed (err: %ld) in file: %s (line: %d) [%s])", err, __FILE__, __LINE__,
                __FUNCTION__);
        writeLog(errLog);
    }
...
bool OptionsDialog::installWindowEventHandler()
{
    ...
    err =
        InstallWindowEventHandler(this->optionsDialogWindow, inEventHandler, inNumTypes, inEventTypeSpecList,
                                  inUserData, &(this->windowEventsHandlerRef));
    if (err != noErr) {
        char errLog[256];
        sprintf(errLog, "InstallWindowEventHandler failed for window events (err: %ld) in file: %s (line: %d) [%s])",
    ...
    err =
        InstallWindowEventHandler(this->optionsDialogWindow, inEventHandler, inNumTypes, inEventTypeSpec, inUserData,
                                  &(this->windowControlEventsHandlerRef));
    if (err != noErr) {
        char errLog[256];
        sprintf(errLog, "InstallWindowEventHandler failed for control events (err: %ld) in file: %s (line: %d) [%s])",
                err, __FILE__, __LINE__, __FUNCTION__);
        writeLog(errLog);
        return success;
    }
...
bool OptionsDialog::populateFontMenu()
{
    ...
    if (err != noErr) {
        char errLog[256];
        sprintf(errLog, "CreateNewMenu failed (err: %ld) in file: %s (line: %d) [%s])", err, __FILE__, __LINE__,
                __FUNCTION__);
        writeLog(errLog);
        return false;
    }
    ...
    if ((err != noErr) || (!fontPopUpControl)) {
        char errLog[256];
        sprintf(errLog, "GetControlByID returned no fontPopUpControl (err: %ld) in file: %s (line: %d) [%s])", err,
                __FILE__, __LINE__, __FUNCTION__);
        writeLog(errLog);
        return success;
    }
