...
bool VisualFile::initWithDirectoryOfTemporaryItems()
{
    ...
    HRESULT hr =
        SHGetSpecialFolderPathW(NULL, (LPWSTR) filePathWin, CSIDL_INTERNET_CACHE, createFolderIfNotAlreadyThere);
    if (!hr) {
        success = false;
    }
#endif
...
bool VisualFile::initWithUserDesktopDirectory()
{
    ...
    BOOL createFolderIfNotAlreadyThere = true;
    HRESULT hr =
        SHGetSpecialFolderPathW(NULL, (LPWSTR) this->filePathWin, CSIDL_DESKTOPDIRECTORY,
                                createFolderIfNotAlreadyThere);
    if (!hr) {
        success = false;
    }
#endif
...
VisualFile *VisualFile::clone(void) const const
{
...
bool VisualFile::appendFileName(VisualString & aFileName)
{
    ...
    CFURLRef urlWithFileName =
        CFURLCreateCopyAppendingPathComponent(kCFAllocatorDefault, url, aFileName.getCharactersPointer(), isDir);
    CFRelease(url);
    FSRef newFsRef;
    ...
    if (!CFURLGetFSRefSuccess) {
        ...
        if (fileNameConstChars == NULL) {
            osErr =
                FSCreateFileUnicode(&prevFsRef, fileNameLength, fileNameChars, whichInfo, catalogInfo, &(this->fsRef),
                                    &newSpec);
        } else {
            osErr =
                FSCreateFileUnicode(&prevFsRef, fileNameLength, fileNameConstChars, whichInfo, catalogInfo,
                                    &(this->fsRef), &newSpec);
        }
        if (osErr != noErr) {
            osStatus = 1000;
...
bool VisualFile::getData(void **dataOut, size_t & size)
{
    ...
    while (readOn == true) {
        ...
        if (readDataValue != 1) {
            ...
            if (debug == true) {
                sprintf(logStr, "OK: readData (cnt: %d, totalBytes: %d, req: %d, read: %d)", bufferFillCount,
                        numberOfBytesRead, numberOfBytesToReadPerFill, actualCount);
                writeLog(logStr);
...
sint8 VisualFile::readData(char *buffer, uint32 & numberOfbytes, uint32 startOffset)
{
    ...
    OSErr osErr =
        FSReadFork(this->currForkRefNum, fsFromStart, startOffset, numberOfbytesRequested, buffer,
                   reinterpret_cast < ByteCount * >(&numberOfbytes));
    if (osErr == noErr) {
        retVal = 1;
...
void VisualFile::getFilePath(VisualString & filePath) const const
{
#if TARGET_OS_MAC
    CFURLRef url = CFURLCreateFromFSRef(kCFAllocatorDefault, &(this->fsRef));
...
bool VisualFile::getSize(size_t & size) const const
{
    bool success = false;
    size = 0;
#if TARGET_OS_MAC
