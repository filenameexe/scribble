...
bool VisualTextureContainer::initWithEncodedData(const char *const bufferData, size_t size)
{
    ...
    status =
        bitmap->LockBits(&rect, Gdiplus::ImageLockModeRead | Gdiplus::ImageLockModeUserInputBuf, PixelFormat32bppPARGB,
                         bitmapData);
    ...
    CFDataRef dataRef =
        CFDataCreateWithBytesNoCopy(kCFAllocatorDefault, (UInt8 *) bufferData, (CFIndex) size, kCFAllocatorDefault);
    ...
    if (debug == true) {
        interleavedARGBColorPixelBuffer =
            VisualColorTools::createARGBCheckPixels(this->textureRect.width, this->textureRect.height);
    } else {
        interleavedARGBColorPixelBuffer = static_cast < PixelColor * >(aPixelBuffer);
    }
    success =
        this->initWithARGBPixelData(interleavedARGBColorPixelBuffer, this->imageRect.width, this->imageRect.height);
...
bool VisualTextureContainer::initWithARGBPixelData(PixelColor * argbPixels, size_t imageWidth, size_t imageHeight,
                                                   bool debug)
{
    ...
    success =
        theVisualGraphics->copyARGBBitmapDataToTexture(this->textureName, this->imageRect.width, this->imageRect.height,
                                                       this->useRectExtension,
                                                       const_cast < const uint32 ** >(&(argbPixels)), debug);
...
bool VisualTextureContainer::initWithBGRAPixelData(PixelColor * bgraPixels, size_t imageWidth, size_t imageHeight,
                                                   bool debug)
{
...
bool VisualTextureContainer::initWithStyledString(VisualStyledString & styledString)
{
    ...
    success =
        getDimensionsOfCocoaStringBitmap((void *) aVisualString, (void *) &stringStyle, &(this->imageRect.width),
                                         &(this->imageRect.height), maxPixelWidth, maxPixelHeight, alignmentStr);
    if (!success) {
        return success;
    }
    ...
    success =
        getCocoaStringBitmapData((void *) &styledString, this->textureRect.width, this->textureRect.height,
                                 alignmentStr, &(pixelBuffer));

    success =
        theVisualGraphics->copyARGBBitmapDataToTexture(this->textureName, this->textureRect.width,
                                                       this->textureRect.height, this->useRectExtension,
                                                       const_cast < const uint32 ** >(&(pixelBuffer)));
    ...
    success =
        theVisualGraphics->makeTextureOfStringWin(stringValueRef, styledString.getNumberOfCharacters(),
                                                  this->textureName, this->textureRect.width, this->textureRect.height,
                                                  this->imageRect.width, this->imageRect.height,
                                                  stringStyle.fontNameStr, (uint16) stringStyle.fontSize, red, green,
                                                  blue, stringStyle.horizontalAlignment, maxPixelWidth, maxPixelHeight);
...
bool VisualTextureContainer::initWithFramebuffer(const BottomLeftPositionedPixelRect & clipRect)
{
    ...
    theVisualGraphics->copyFramebufferToTexture(this->textureName, this->useRectExtension, clipRect, pixelFormat,
                                                dataType);
...
#if __BIG_ENDIAN__
        VisualColorTools::convertInterleavedPixels1234To4321(pixelBuffer,
...
PixelColor *VisualTextureContainer::createReadPixels(const uint16 format, const uint16 type)
{
    ...
    coord.y =
        (aCamera.getMaxBottomCoord() +
         theVisualGraphics->yPixelToCoord(this->textureRect.height, aCamera)) * scaleFactor;
    ...
    coord.x =
        (aCamera.getMaxLeftCoord() + theVisualGraphics->xPixelToCoord(this->textureRect.width, aCamera)) * scaleFactor;
    coord.y = aCamera.getMaxBottomCoord() * scaleFactor;
    coord.z = aCamera.getMaxNearPos();
    ...
    if ((format == kGL_RGBA) || (format == kGL_BGRA)) {
        prevPixels =
            (uint32 *) calloc((numberOfBytesPerRow / numberOfBytesPerPixel) * this->textureRect.height,
                              numberOfBytesPerPixel);
    }
    theVisualGraphics->readPixels(aCamera.getMaxLeftCoord(), aCamera.getMaxBottomCoord(), this->textureRect.width,
                                  this->textureRect.height, &prevPixels, format, type, aCamera);
    ...
    theVisualGraphics->readPixels(aCamera.getMaxLeftCoord(), aCamera.getMaxBottomCoord(), this->textureRect.width,
                                  this->textureRect.height, &pixels, format, type, aCamera);

    // restore previous pixels
    theVisualGraphics->drawPixels(&prevPixels, aCamera.getMaxLeftCoord(), aCamera.getMaxBottomCoord(),
                                  this->textureRect.width, this->textureRect.height, format, type);
...
uint32 VisualTextureContainer::getTextureName() const const
{
    return this->textureName;
}


size_t VisualTextureContainer::getTextureWidth() const const
{
    return this->textureRect.width;
}


size_t VisualTextureContainer::getTextureHeight() const const
{
    return this->textureRect.height;
}


double VisualTextureContainer::getTextureLogicalWidth() const const
{
    return this->logicalSize.width;
}


double VisualTextureContainer::getTextureLogicalHeight() const const
{
    return this->logicalSize.height;
}


size_t VisualTextureContainer::getImageWidth() const const
{
    return this->imageRect.width;
}


size_t VisualTextureContainer::getImageHeight() const const
{
    return this->imageRect.height;
}


bool VisualTextureContainer::getUseRectExtension() const const
{
...
void VisualTextureContainer::applyConvolutionFilter(const VisualConvolutionFilter & aConvolutionFilter)
{
    ...
    if (theVisualGraphics->doesSupportGLConvolutionFilter()) {
        ...
        if ((pixelFormat == kGL_RGBA) || (pixelFormat == kGL_BGRA)) {
            ...
            sprintf(errStr, "unknown format %d in file: %s (line: %d) [%s])", pixelFormat, __FILE__, __LINE__,
                    __FUNCTION__);
            writeLog(errStr);
            return;
        }

        if ((dataType == kGL_UNSIGNED_INT_8_8_8_8_REV) || (dataType == kGL_UNSIGNED_INT_8_8_8_8)
            || (dataType == kGL_UNSIGNED_BYTE)) {
            numberOfBytesPerChannel = 1;        // // 1 byte (== 8 bits) per color/channel
        } else {
        ...
        aCamera.setOrthographicProjection();
        theVisualGraphics->readPixels(aCamera.getMaxLeftCoord(), aCamera.getMaxBottomCoord(), this->textureRect.width,
                                      this->textureRect.height, &prevPixels, pixelFormat, dataType, aCamera);
        ...
        theVisualGraphics->drawPixels(&pixels, aCamera.getMaxLeftCoord(), aCamera.getMaxBottomCoord(),
                                      this->imageRect.width, this->imageRect.height, pixelFormat, dataType,
                                      &aConvolutionFilter);
        ...
        // restore previous pixels
        theVisualGraphics->drawPixels(&prevPixels, aCamera.getMaxLeftCoord(), aCamera.getMaxBottomCoord(),
                                      this->textureRect.width, this->textureRect.height, pixelFormat, dataType);
        ...
        aConvolutionFilter.applyToPixelData(pixels, this->imageRect.width, this->imageRect.height, pixelFormat,
                                            dataType, &filteredPixels);
        free(pixels);
        pixels = NULL;
        bool success = false;
        const PixelColor *constFilteredPixels = const_cast < const PixelColor * >(filteredPixels);
        success =
            theVisualGraphics->copyARGBBitmapDataToTexture(this->textureName, this->imageRect.width,
                                                           this->imageRect.height, this->useRectExtension,
                                                           &constFilteredPixels);
...
void VisualTextureContainer::resample(const PixelRect & pixelRect)
{
    ...
    theVisualGraphics->resample(aPixelFormat, this->imageRect.width, this->imageRect.height, dataTypeIn, pixels,
                                pixelRect.width, pixelRect.height, dataTypeOut, &pixelsOut);
    free(pixels);
