...
void VisualConvolutionFilter::applyToPixelData(const PixelColor * inPixelData, uint32 imageWidth, uint32 imageHeight,
                                               int pixelFormat, int pixelDataType, PixelColor ** outPixelData,
                                               uint16 numberOfInterations) const const
{
...
uint32 VisualConvolutionFilter::clamp(uint32 inVal) const const
{
    if (inVal > 255)
        return 255;
    if (inVal < 0)
