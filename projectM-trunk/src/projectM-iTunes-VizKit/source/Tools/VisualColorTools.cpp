...
void VisualColorTools::getColorComponentValues(const PixelColor & pixelColor, uint8 & alpha, uint8 & red, uint8 & green,
                                               uint8 & blue)
{
...
PixelColor VisualColorTools::getPixelColor(const uint8 & alpha, const uint8 & red, const uint8 & green,
                                           const uint8 & blue)
{
...
PixelColor VisualColorTools::getMeanRGBPixelColor(const PixelColor & colorX, const PixelColor & colorY,
                                                  const uint32 & count)
{
...
PixelColor *VisualColorTools::createCheckPixels(uint32 width, uint32 height, const char *const format,
                                                const VertexColor & checkColor)
{
    ...
    if (strcmp(format, "ARGB") == 0) {
        ...
        char errStr[256];
        sprintf(errStr, "Err: unknown colorChannelFormat in file: %s (line: %d) [%s])", __FILE__, __LINE__,
                __FUNCTION__);
        writeLog(errStr);
        return NULL;
    }
    ...
    if (checkColor.r == 1.0 && checkColor.g == 0.0 && checkColor.b == 0.0 && checkColor.a == 1.0
        && checkColor.red == 1.0 && checkColor.green == 0.0 && checkColor.blue == 0.0 && checkColor.alpha == 1.0) {
        color = 1;              // red
    ...
    for (i = 0; i < height; i++) {
        for (j = 0; j < width; j++) {
            ...
            default:
                char errStr[256];
                sprintf(errStr, "unknown colorChannelFormat in file: %s (line: %d) [%s])", __FILE__, __LINE__,
                        __FUNCTION__);
                writeLog(errStr);
            }
            cnt++;
