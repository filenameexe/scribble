...
double VisualTimeline::getMinValue() const const
{
    return this->minValue;
}


double VisualTimeline::getMaxValue() const const
{
    return this->maxValue;
}


double VisualTimeline::getDistance(void) const const
{
...
uint32 VisualTimeline::getDurationInMilliseconds() const const
{
...
RepeatMode VisualTimeline::getRepeatMode() const const
{
...
bool VisualTimeline::setCurrentValue(double newCurrValue)
{
    ...
    if (this->movingDirection == kAscending) {
        this->offsetMilliseconds =
            (uint32) ((double) this->durationInMilliseconds * ((newCurrValue - this->minValue) / this->distance));
        if (this->debugMode == true) {
        ...
        this->offsetMilliseconds =
            (uint32) ((double) this->durationInMilliseconds * ((this->maxValue - newCurrValue) / this->distance));
        if (this->debugMode == true) {
...
TimelineUpdateResult VisualTimeline::update()
{
    ...
    if (this->debugMode == true) {
        sprintf(logStr,
                "VisualTimeline::update1: this->currentValue: %f, this->offsetMilliseconds: %d, this->elapsedMilliseconds: %d",
    ...
    this->elapsedMilliseconds =
        VisualTiming::getElapsedMilliSecsSinceReset(*(this->durationIdentifier)) + this->offsetMilliseconds;
    if (this->elapsedMilliseconds >= this->durationInMilliseconds) {
        if (this->debugMode == true) {
            sprintf(logStr,
                    "VisualTimeline::update2: this->elapsedMilliseconds >= this->durationInMilliseconds (%d >= %d)",
        ...
        if (this->debugMode == true) {
            sprintf(logStr,
                    "VisualTimeline::update3: this->elapsedMilliseconds < this->durationInMilliseconds (%d < %d)",
                    this->elapsedMilliseconds, this->durationInMilliseconds);
            writeLog(logStr);
        }
        ...
        if (this->movingDirection == kAscending) {
            if (this->debugMode == true) {
                sprintf(logStr,
                        "VisualTimeline::update4: (double)this->elapsedMilliseconds / (double)this->durationInMilliseconds: %f (%d / %d: %f)",
            ...
            interpolationValue =
                this->visualInterpolation->getValueAtPosition((double) this->elapsedMilliseconds /
                                                              (double) this->durationInMilliseconds);
        } else if (this->movingDirection == kDescending) {
            if (this->debugMode == true) {
                sprintf(logStr,
                        "VisualTimeline::update5: 1.0 - (double)this->elapsedMilliseconds / (double)this->durationInMilliseconds: %f (1.0 - %d / %d: %f)",
            ...
            interpolationValue =
                this->visualInterpolation->getValueAtPosition(1.0 -
                                                              (double) this->elapsedMilliseconds /
                                                              (double) this->durationInMilliseconds);
        }
...
double VisualTimeline::getCurrentValue() const const
{
...
MovingDirection VisualTimeline::getMovingDirection() const const
{
...
void VisualTimeline::toggleMovingDirection()
{
    ...
    if (this->debugMode == true) {
        ...
        if (this->movingDirection == kAscending) {
            sprintf(logStr,
                    "VisualTimeline::toggleMovingDirection: kAscending (this->offsetMilliseconds: %d) (this->durationInMilliseconds: %d)",
                    this->offsetMilliseconds, this->durationInMilliseconds);
        } else if (this->movingDirection == kDescending) {
            sprintf(logStr,
                    "VisualTimeline::toggleMovingDirection: kDescending (this->offsetMilliseconds: %d) (this->durationInMilliseconds: %d)",
                    this->offsetMilliseconds, this->durationInMilliseconds);
        }
        writeLog(logStr);
    }
...
void VisualTimeline::convertTimelineUpdateResultToString(const TimelineUpdateResult aResult, char *outString)
{
    ...
    default:
        char errLog[256];
        sprintf(errLog, "unhandled TimelineUpdateResult %d in file: %s (line: %d) [%s])", aResult, __FILE__, __LINE__,
                __FUNCTION__);
        writeLog(errLog);
