...
VisualAnimationComponent *VisualAnimation::clone(void) const const
{
...
{
    char errLog[256];
    sprintf(errLog, "Err: VisualAnimation is unable to handle collection animations in file: %s (line: %d) [%s])",
...
void VisualAnimation::stop()
{
    ...
    if (this->collectionIdentifier != NULL) {
        ...
        if (animationCollection != NULL) {
            ...
            char errLog[256];
            sprintf(errLog, "CollectionAnimation not found in in file: %s (line: %d) [%s])", __FILE__, __LINE__,
                    __FUNCTION__);
            writeLog(errLog);
        }
...
uint32 VisualAnimation::getDurationInMilliseconds() const const
{
...
double VisualAnimation::getCurrentValue() const const
{
...
double VisualAnimation::getStartValue() const const
{
...
double VisualAnimation::getStopValue() const const
{
...
uint32 VisualAnimation::getStartDelayInMilliseconds() const const
{
...
void VisualAnimation::
setCallbackToPerformAnyAdditionalAction(VisualAnimationPerformAnyAdditionalActionCallback theCallback, void *userData)
{
...
AnimatedProperty VisualAnimation::getAnimatedProperty() const const
{
    return this->animatedProperty;
}


MovingDirection VisualAnimation::getMovingDirection(void) const const
{
    return this->theTimeline->getMovingDirection();
}


RepeatMode VisualAnimation::getRepeatMode(void) const const
{
...
bool VisualAnimation::isRunning() const const
{
...
bool VisualAnimation::isReadyToDie(void) const const
{
...
void VisualAnimation::preparePriorToAddingToAsset(VisualAsset & visualAsset)
{
    ...
    VisualAnimationQueue::removeVisualAnimationsWithOwnerIdentifier(visualAsset.getIdentifier(),
                                                                    this->animatedProperty);
...
const VisualAsset *const VisualAnimation::getEnclosingAsset(void) const const
{
...
AnimationSpeed VisualAnimation::getSpeed(void) const const
{
    return this->speed;
}
                                                                    ...
                                                                    AnimatedProperty anAnimatedProperty)
{
    ...
    if (anAnimatedProperty == kAnimatedLocation) {
        adjustedDistanceAndDuration =
            this->theTimeline->setCurrentValue(visualAsset.
                                               getCurrentAnimationValueForAnimatedLocation(this->
                                                                                           startValueVisualStagePosition,
                                                                                           this->
                                                                                           stopValueVisualStagePosition));
    } else {
        adjustedDistanceAndDuration =
            this->theTimeline->setCurrentValue(visualAsset.getCurrentAnimationValueOfProperty(anAnimatedProperty));
    }
    // movingDirection might need to change (we want to arrive at the stop position by way of the shortest distance)
    MovingDirection movingDirection = this->theTimeline->getMovingDirection();
    double theCurrentValue = this->theTimeline->getCurrentValue();
