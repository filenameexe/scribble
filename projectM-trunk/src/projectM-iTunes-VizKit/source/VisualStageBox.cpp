...
void VisualStageBox::calcCoords(const VisualCamera & aCamera)
{
    ...
    this->coordWidth =
        size.width * ((double) this->contentPixelWidth / (double) theVisualGraphics->getCanvasPixelWidth());
    //printf("hwtest coordWidth2: %f\n", this->coordWidth);
    this->coordHeight =
        size.height * ((double) this->contentPixelHeight / (double) theVisualGraphics->getCanvasPixelHeight());
    ...
    if (this->stagePosition.minMarginBottom != 0.0) {
        if (this->stagePosition.minMarginBottomUnit == kPercent) {
            ...
            minMarginBottomCoord =
                theVisualGraphics->yPixelToCoord((uint16) this->stagePosition.minMarginBottom, aCamera);
        }
    }
    ...
    if (this->stagePosition.minMarginRight != 0.0) {
        if (this->stagePosition.minMarginRightUnit == kPercent) {
            ...
            minMarginRightCoord =
                theVisualGraphics->xPixelToCoord((uint16) this->stagePosition.minMarginRight, aCamera);
        }
    }
    ...
    if ((this->scalingBehaviour & kScalingAllowed) == kScalingAllowed) {
        ...
        if (this->stagePosition.minWidth > 0.0) {
            prevCoordVal = this->coordWidth;
            if ((this->stagePosition.minWidthUnit == kPercent)
                && ((this->coordWidth / size.width * 100.0) < this->stagePosition.minWidth)) {
            ...
            } else if ((this->stagePosition.minWidthUnit == kPixel)
                       && (static_cast < double >(theVisualGraphics->xCoordToPixel(this->coordWidth, aCamera)) <
                           this->stagePosition.minWidth)) {
        ...
        if (this->stagePosition.minHeight > 0.0) {
            prevCoordVal = this->coordHeight;
            if ((this->stagePosition.minHeightUnit == kPercent)
                && ((this->coordHeight / size.height * 100.0) < this->stagePosition.minHeight)) {
            ...
            } else if ((this->stagePosition.minHeightUnit == kPixel)
                       && (static_cast < double >(theVisualGraphics->yCoordToPixel(this->coordHeight, aCamera)) <
                           this->stagePosition.minHeight)) {
        ...
        if (this->stagePosition.maxWidth > 0.0) {
            prevCoordVal = this->coordWidth;
            if ((this->stagePosition.maxWidthUnit == kPercent)
                && ((this->coordWidth / size.width * 100.0) > this->stagePosition.maxWidth)) {
            ...
            } else if ((this->stagePosition.maxWidthUnit == kPixel)
                       && (static_cast < double >(theVisualGraphics->xCoordToPixel(this->coordWidth, aCamera)) >
                           this->stagePosition.maxWidth)) {
        ...
        if (this->stagePosition.maxHeight > 0.0) {
            prevCoordVal = this->coordHeight;
            if ((this->stagePosition.maxHeightUnit == kPercent)
                && ((this->coordHeight / size.height * 100.0) > this->stagePosition.maxHeight)) {
            ...
            } else if ((this->stagePosition.maxHeightUnit == kPixel)
                       && (static_cast < double >(theVisualGraphics->yCoordToPixel(this->coordHeight, aCamera)) >
                           this->stagePosition.maxHeight)) {
...
*/
        ...
        if (1 == 2) {
            ...
            if (this->stagePosition.verticalAlignment == kTopAligned) {
                ...
                bottom =
                    aCamera.getMaxBottomCoord() + marginBottomCoord + (((size.height - marginBottomCoord) / 2.0) -
                                                                       (this->coordHeight / 2.0));
                if ((minMarginBottomCoord > marginBottomCoord)
                    && ((aCamera.getMaxBottomCoord() - bottom) < minMarginBottomCoord)) {
                    bottom = aCamera.getMaxBottomCoord() + minMarginBottomCoord;
                }
            } else if (this->stagePosition.verticalAlignment == kBottomAligned) {
            ...
            if (this->stagePosition.horizontalAlignment == kLeftAligned) {
                ...
                left =
                    aCamera.getMaxLeftCoord() + marginLeftCoord + (((size.width - marginLeftCoord) / 2.0) -
                                                                   (this->coordWidth / 2.0));
            ...
            if (top > (aCamera.getMaxTopCoord() - minMarginTopCoord)) {
                ...
                if ((this->scalingBehaviour & kPreserveAspectRatio) == kPreserveAspectRatio) {
                    ...
                    if (this->stagePosition.horizontalAlignment == kLeftAligned) {
                        ...
                        left =
                            (aCamera.getMaxLeftCoord() + marginLeftCoord + (size.width / 2.0)) -
                            (this->coordWidth / 2.0);
...
ScalingBehaviour VisualStageBox::getScalingBehaviour() const const
{
    return this->scalingBehaviour;
}


double VisualStageBox::getScaleFactor() const const
{
...
uint32 VisualStageBox::getContentPixelWidth() const const
{
    return this->contentPixelWidth;
}


uint32 VisualStageBox::getContentPixelHeight() const const
{
    return this->contentPixelHeight;
}


VisualStagePosition VisualStageBox::getVisualStagePosition() const const
{
...
VertexChain *VisualStageBox::getVertexChain(const VisualItemIdentifier & vertexChainName)
{
    ...
    if (mapIt != this->vertexChainMap.end()) {
        ...
        char errLog[64];
        sprintf(errLog, "unable to find vertex chain to return in file: %s (line: %d) [%s])", __FILE__, __LINE__,
                __FUNCTION__);
        writeLog(errLog);
    }
...
VertexChain *VisualStageBox::getVertexChain(size_t vertexIdx)
{
    ...
    if (vertexIdx > this->vertexChainMap.size() - 1)
        return NULL;
    size_t count = 0;
...
size_t VisualStageBox::getNumberOfVertexChains(void) const const
{
    return this->vertexChainMap.size();
}


bool VisualStageBox::hasVertexChain(const VisualItemIdentifier & vertexChainName) const const
{
    bool result = false;
    ConstVertexChainMapIterator mapIt = this->vertexChainMap.find(vertexChainName);
    if (mapIt != this->vertexChainMap.end()) {
...
VisualVertex *VisualStageBox::createVertex(double xPos, double yPos, double zPos, double aTexCoordSPos,
                                           double aTexCoordTPos, VertexColor anRGBAColor)
{
        ...
        for (VertexChainIterator chainIt = mapIt->second->begin(); chainIt != mapIt->second->end(); chainIt++) {
            if ((*chainIt)->vertexPosition.relCoord.x != -1.0) {
                (*chainIt)->vertexPosition.coord.x =
                    this->getLeftCoord() + (this->getCoordWidth() * (*chainIt)->vertexPosition.relCoord.x);
            ...
            if ((*chainIt)->vertexPosition.relCoord.y != -1.0) {
                (*chainIt)->vertexPosition.coord.y =
                    this->getBottomCoord() + (this->getCoordHeight() * (*chainIt)->vertexPosition.relCoord.y);
            ...
            if ((*chainIt)->texCoordPosition.relCoord.s != -1.0) {
                ...
                if (anImage != NULL) {
                    ...
                    (*chainIt)->texCoordPosition.coord.s =
                        anImage->getLogicalWidth() * (*chainIt)->texCoordPosition.relCoord.s;
            ...
            if ((*chainIt)->texCoordPosition.relCoord.t != -1.0) {
                const VisualImage *const anImage = this->assetRef->getImage();
                if (anImage != NULL) {
                    (*chainIt)->texCoordPosition.coord.t =
                        anImage->getLogicalHeight() * (*chainIt)->texCoordPosition.relCoord.t;
                }
            }
        }
    }
