...
void VisualAudioLab::init()
{
    ...
    beatsRecordedInBeatAnalyseTimeFrame =
        (BeatGuess *) malloc(VisualAudioLab::maxNumberOfBeatsRecordedInAnalyseTimeframe * sizeof(BeatGuess));
...
void VisualAudioLab::processAudioData(uint8 numWaveformChannels,
                                      ...
                                      uint16 numSpectrumEntries, const uint8 * const currSpectrumData)
{
...
void VisualAudioLab::processWaveformData(uint8 numWaveformChannels, uint16 numWaveformEntries,
                                         const uint8 * const currWaveformData)
{
    ...
    sampleRate =
        static_cast < uint32 > (static_cast <
                                float >(VisualAudioLab::numberOfAudioWaveformDataEntries) * (1000.0 / static_cast <
                                                                                             float
                                                                                             >(elapsedMilliseconds)));
    ...
    theVisualAudioLab->rmsBuffer[theVisualAudioLab->currRMSValue].timeInMillisecondsSinceStartOfTrack =
    ...
    for (index = 0; index < numWaveformEntries; index++) {
        theVisualAudioLab->tappingBuffer[tappingBufferHighWatermarkIdx] =
            static_cast < double >(theVisualAudioLab->waveformDataMono[theVisualAudioLab->currMusicDataHistory][index]);
...
void VisualAudioLab::addToBeatGapHistogram(uint16 gapInMilliseconds)
{
    ...
    if (beatGapHistogramIdx > (VisualAudioLab::maxNumberOfBeatHistogramBins - 1))
        return;
...
uint32 VisualAudioLab::getBestBeatMSInterval()
{
    ...
    if (beatIndex == 0) {
        bestBeatMSInterval =
            ...
            beatsRecordedInBeatAnalyseTimeFrame[VisualAudioLab::maxNumberOfBeatsRecordedInAnalyseTimeframe].
...
void VisualAudioLab::processSpectrumData(const uint8 numSpectrumChannels, const uint16 numSpectrumEntries,
...
uint8 VisualAudioLab::getRMSIntensity()
{
    uint8 intensity;
    intensity =
        (uint8) ((theVisualAudioLab->rmsMono[theVisualAudioLab->currMusicDataHistory] / theVisualAudioLab->rmsMonoMax) *
                 100.0);
    return intensity;
}
...
uint8 VisualAudioLab::getWaveformDataOfChannelAtIndexWithHistoryNum(const uint16 channel, const uint16 index,
