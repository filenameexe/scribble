...
bool VisualAsset::operator<(const VisualAsset & other) const const
{
...
VisualAsset *VisualAsset::clone(void) const const
{
...
VisualStageBox *VisualAsset::getBox() const const
{
...
VisualCamera & VisualAsset::getCamera(void) const const
{
    return *(this->camera);
}


VisualStagePosition VisualAsset::getPosition() const const
{
...
bool VisualAsset::hasImage() const const
{
    if (this->image == NULL) {
        return false;
    }
...
VisualImage *VisualAsset::getImage() const const
{
    return this->image;
}
...
void VisualAsset::draw(const VisualItemIdentifier & vertexChainName) const const
{
...
void VisualAsset::draw() const const
{
    ...
    for (size_t i = 0; i < numberOfVertexChains; i++) {
        if (const_cast < VisualItemIdentifier & >(this->stageBox->getVertexChainIdentifier(i)) !=
            *(this->defaultVertexChainIdentifier)) {
...
void VisualAsset::doDraw(const VertexChain & aVertexChain) const const
{
    this->camera->activate();
...
void VisualAsset::drawBoxOutline() const const
{
    VertexColor yellow;
    yellow.r = 1.0;
    yellow.g = 1.0;
...
double VisualAsset::getScaleFactor() const const
{
...
double VisualAsset::getOpacityValue() const const
{
...
double VisualAsset::getRotationAngle() const const
{
...
{
    VisualAnimationQueue::removeVisualAnimationsWithOwnerIdentifier(this->VisualObject::getIdentifier(),
                                                                    anAnimatedProperty);
}


const VisualAnimationComponent *const VisualAsset::getAnimation(const AnimatedProperty & anAnimatedProperty) const const
{
    return VisualAnimationQueue::getFirstVisualAnimation(this->VisualObject::getIdentifier(), anAnimatedProperty);
}


double VisualAsset::getCurrentAnimationValueOfProperty(const AnimatedProperty & anAnimatedProperty) const const
{
    ...
    default:
        char errLog[256];
        sprintf(errLog, "unknown AnimatedProperty %d in file: %s (line: %d) [%s])", anAnimatedProperty, __FILE__,
...
void VisualAsset::updateLocation(double currPosition)
{
    VisualStagePosition position =
        VisualStageBox::tweenVisualStagePosition(this->startValueVisualStagePosition,
                                                 this->stopValueVisualStagePosition, currPosition, *this->stageBox);
    this->setPosition(position);
}


double VisualAsset::getCurrentAnimationValueForAnimatedLocation(const VisualStagePosition & startPosition,
                                                                const VisualStagePosition & stopPosition) const const
{
