...
void VisualGraphics::setCanvasRect(const TopLeftPositionedPixelRect & aCanvasRect)
{
    ...
    this->canvasRect.bottomLeftPixel.y =
        ((this->surfaceRect.pixelRect.height - aCanvasRect.pixelRect.height) / 2) + this->surfaceRect.bottomLeftPixel.y;
}
...
RelationalRect VisualGraphics::getViewportOrientationAndAspectRatio()
{
    ...
    if ((bottomLeftPositionedPixelRect.pixelRect.height) > (bottomLeftPositionedPixelRect.pixelRect.width)) {
        ...
        relationalRect.aspectRatio =
            (double) bottomLeftPositionedPixelRect.pixelRect.height /
            (double) bottomLeftPositionedPixelRect.pixelRect.width;
...
void VisualGraphics::setOrthographicProjection(double maxLeftCoord, double maxRightCoord, double maxBottomCoord,
                                               double maxTopCoord, double maxNearPos, double maxFarPos,
                                               double zoomFactor)
{
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();

    glOrtho(maxLeftCoord * zoomFactor, maxRightCoord * zoomFactor, maxBottomCoord * zoomFactor,
            maxTopCoord * zoomFactor, maxNearPos, maxFarPos);

    glMatrixMode(GL_MODELVIEW);
}


void VisualGraphics::setPerspectiveProjection(double maxLeftCoord, double maxRightCoord, double maxBottomCoord,
                                              double maxTopCoord, double maxNearPos, double maxFarPos,
                                              double zoomFactor)
{
    glMatrixMode(GL_PROJECTION);
    ...
    glFrustum(maxLeftCoord * zoomFactor, maxRightCoord * zoomFactor, maxBottomCoord * zoomFactor,
              maxTopCoord * zoomFactor, maxNearPos, maxFarPos);
...
{
    glClearColor((GLclampf) canvasBackgroundColor.r, (GLclampf) canvasBackgroundColor.g,
                 (GLclampf) canvasBackgroundColor.b, (GLclampf) canvasBackgroundColor.a);
...
{
    glClearColor((GLclampf) surfaceBackgroundColor.r, (GLclampf) surfaceBackgroundColor.g,
                 (GLclampf) surfaceBackgroundColor.b, (GLclampf) surfaceBackgroundColor.a);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}
...
void VisualGraphics::drawTexture(uint32 textureNumber, const VertexChain * const vertexChain, bool canUseRectExtension,
                                 BlendMode aBlendMode, bool debug)
{
    ...
    for (ConstVertexChainConstIterator it = vertexChain->begin(); it != vertexChain->end(); it++) {
        ...
        if (debug == true) {
            char logStr[128];
            sprintf(logStr, "r: %f, g: %f, b: %f, a: %f\n", (*it)->vertexColor.r, (*it)->vertexColor.g,
                    (*it)->vertexColor.b, (*it)->vertexColor.a);
            writeLog(logStr);
        }
        ...
        if (debug == true) {
            char logStr[128];
            sprintf(logStr, "x: %f, y: %f, z: %f\n", (*it)->vertexPosition.coord.x, (*it)->vertexPosition.coord.y,
                    (*it)->vertexPosition.coord.z);
            writeLog(logStr);
        }
    }
...
void VisualGraphics::drawDebugVertexChain(const VertexChain & vertexChain, const VisualCamera & aCamera)
{
    ...
    for (ConstVertexChainConstIterator it = vertexChain.begin(); it != vertexChain.end(); it++) {
        ...
        sprintf(posStr, "x:%.2f, y:%.2f, z:%.2f", (*it)->vertexPosition.coord.x, (*it)->vertexPosition.coord.y,
                (*it)->vertexPosition.coord.z);
...
void VisualGraphics::drawPixels(PixelColor ** pixelData, double xCoord, double yCoord, uint32 width, uint32 height,
                                uint16 format, uint16 dataType,
                                const VisualConvolutionFilter * const aConvolutionFilter)
{
    ...
    if (useGLConvolutionFilter == true) {
        kernel =
            (GLfloat *) malloc(aConvolutionFilter->getNumberOfKernelValueColumns() *
                               aConvolutionFilter->getNumberOfKernelValueRows() * sizeof(GLfloat));
        ...
        (*glConvolutionFilter2D) (GL_CONVOLUTION_2D, GL_LUMINANCE, aConvolutionFilter->getNumberOfKernelValueColumns(),
                                  aConvolutionFilter->getNumberOfKernelValueRows(), GL_LUMINANCE, GL_FLOAT, kernel);
...
void VisualGraphics::resample(uint16 formatIn, uint32 widthIn, uint32 heightIn, uint16 dataTypeIn,
                              PixelColor * pixelDataIn, uint32 widthOut, uint32 heightOut, uint16 dataTypeOut,
                              PixelColor ** pixelDataOut)
{

    this->setPixelStorageParams();
    gluScaleImage((GLenum) formatIn, (GLint) widthIn, (GLint) heightIn, (GLenum) dataTypeIn, (void *) pixelDataIn,
                  (GLint) widthOut, (GLint) heightOut, (GLenum) dataTypeOut, (void *) (*pixelDataOut));
    return;
...
void VisualGraphics::readPixels(double xCoord, double yCoord, uint32 width, uint32 height, uint32 ** pixelData,
                                uint16 format, uint16 dataType, const VisualCamera & aCamera)
{
    uint16 xPos, yPos;
...
bool VisualGraphics::makeTextureOfStringWin(wchar_t * stringValue,
                                            ...
                                            uint8 green,
                                            uint8 blue,
                                            HorizontalAlignment alignment, uint32 maxPixelWidth, uint32 maxPixelHeight)
{
    //#define useGetDIBits
    ...
    case (kRightAligned):
        ...
        break;
    default:
        sprintf(errStr, "unknown switch case (%d) in file: %s (line: %d) [%s])", alignment, __FILE__, __LINE__,
                __FUNCTION__);
        writeLog(errStr);
    }
    ...
    if (strlen(fontName) > 32) {
        sprintf(errStr, "length of fontName must not exceed 32 characters but is %d in file: %s (line: %d) [%s])",
                strlen(fontName), __FILE__, __LINE__, __FUNCTION__);
        writeLog(errStr);
    ...
    if (!trackTitleFont) {
        sprintf(errStr, "Err after CreateFontIndirect() in file: %s (line: %d) [%s])", __FILE__, __LINE__,
                __FUNCTION__);
        writeLog(errStr);
    }
    ...
    if (textDrawRect.bottom == 0) {
        sprintf(errStr, "Err (textDrawRect.bottom == 0) in file: %s (line: %d) [%s])", __FILE__, __LINE__,
                __FUNCTION__);
        writeLog(errStr);
    }
    if (drawTextResult == 0) {
    ...
    if ((maxPixelWidth > 0) && (maxPixelHeight > 0)) {
        while ((stringSizeWidth > maxPixelWidth) || (stringSizeHeight > maxPixelHeight)) {
            ...
            if (drawTextResult == 0) {
                sprintf(errStr, "Err (drawTextResult == 0) in file: %s (line: %d) [%s])", __FILE__, __LINE__,
                        __FUNCTION__);
                writeLog(errStr);
                return false;
            }
    ...
    if (!hbmpTemp) {
        sprintf(errStr, "err after CreateCompatibleBitmap() in file: %s (line: %d) [%s])", __FILE__, __LINE__,
                __FUNCTION__);
        writeLog(errStr);
    }
#endif
    ...
    this->copyARGBBitmapDataToTexture(textureNumber, stringSizeWidth, stringSizeHeight,
                                      this->canUseTextureRectExtension(), &pixels);
...
void VisualGraphics::drawSpot(const double xNum, const double yNum, const double r, const double g, const double b,
                              uint16 waveformIntensityVal, double intensity, const uint8 tailSize)
{
...
void VisualGraphics::drawProjectionMetrics(const VisualCamera & aCamera)
{
    ...
    for (uint32 i = 0; i < numberOfMarks; i++) {
        glRasterPos3d(aCamera.getMaxLeftCoord(), aCamera.getMaxBottomCoord(),
                      nearPos - ((distance / numberOfMarks) * i));
        sprintf(str, "%.2f", nearPos - ((distance / numberOfMarks) * i));
        this->drawCStringWithGL(str, strlen(str));
    }
    for (uint32 i = 0; i < numberOfMarks; i++) {
        glRasterPos3d(aCamera.getMaxRightCoord(), aCamera.getMaxBottomCoord(),
                      nearPos - ((distance / numberOfMarks) * i));
        sprintf(str, "%.2f", nearPos - ((distance / numberOfMarks) * i));
        this->drawCStringWithGL(str, strlen(str));
    }
    ...
    for (uint32 i = 0; i < numberOfMarks; i++) {
        glRasterPos3d(aCamera.getMaxLeftCoord(), aCamera.getMaxBottomCoord() + ((verticalDistance / numberOfMarks) * i),
                      nearPos);
        sprintf(str, "%.2f", aCamera.getMaxBottomCoord() + ((verticalDistance / numberOfMarks) * i));
        this->drawCStringWithGL(str, strlen(str));
    }
    for (uint32 i = 0; i < numberOfMarks; i++) {
        glRasterPos3d(aCamera.getMaxRightCoord() - 0.2,
                      aCamera.getMaxBottomCoord() + ((verticalDistance / numberOfMarks) * i), nearPos);
        sprintf(str, "%.2f", aCamera.getMaxBottomCoord() + ((verticalDistance / numberOfMarks) * i));
        this->drawCStringWithGL(str, strlen(str));
    }
    double horizontalDistance = aCamera.getCoordWidth();
    for (uint32 i = 0; i < numberOfMarks; i++) {
        glRasterPos3d(aCamera.getMaxLeftCoord() + ((horizontalDistance / numberOfMarks) * i),
                      aCamera.getMaxBottomCoord(), nearPos);
        sprintf(str, "%.2f", aCamera.getMaxLeftCoord() + ((horizontalDistance / numberOfMarks) * i));
        this->drawCStringWithGL(str, strlen(str));
    }
    for (uint32 i = 0; i < numberOfMarks; i++) {
        glRasterPos3d(aCamera.getMaxLeftCoord() + ((horizontalDistance / numberOfMarks) * i),
                      aCamera.getMaxTopCoord() - 0.1, nearPos);
        sprintf(str, "%.2f", aCamera.getMaxLeftCoord() + ((horizontalDistance / numberOfMarks) * i));
        this->drawCStringWithGL(str, strlen(str));
    }
    for (uint32 i = 0; i < numberOfMarks; i++) {
    ...
    for (uint32 i = 0; i < numberOfMarks; i++) {
        glRasterPos3d(aCamera.getMaxRightCoord() - 0.2,
                      aCamera.getMaxBottomCoord() + ((verticalDistance / numberOfMarks) * i), farPos);
        sprintf(str, "%.2f", aCamera.getMaxBottomCoord() + ((verticalDistance / numberOfMarks) * i));
        this->drawCStringWithGL(str, strlen(str));
    }
    for (uint32 i = 0; i < numberOfMarks; i++) {
        glRasterPos3d(aCamera.getMaxLeftCoord() + ((horizontalDistance / numberOfMarks) * i),
                      aCamera.getMaxBottomCoord(), farPos);
        sprintf(str, "%.2f", aCamera.getMaxLeftCoord() + ((horizontalDistance / numberOfMarks) * i));
        this->drawCStringWithGL(str, strlen(str));
    }
    for (uint32 i = 0; i < numberOfMarks; i++) {
        glRasterPos3d(aCamera.getMaxLeftCoord() + ((horizontalDistance / numberOfMarks) * i), aCamera.getMaxTopCoord(),
                      farPos);
        sprintf(str, "%.2f", aCamera.getMaxLeftCoord() + ((horizontalDistance / numberOfMarks) * i));
        this->drawCStringWithGL(str, strlen(str));
    }
...
void VisualGraphics::drawWaveform(const sint16 historyNum, const uint16 maxNumberOfHistories,
                                  const uint32 numberOfWaveformEntries, sint16 ** waveformDataMonoArray,
                                  const VisualCamera & aCamera)
{
    ...
    for (i = 0; i < maxNumberOfHistories; i++) {
        ...
        for (k = 0; k < numberOfWaveformEntries; k++) {
            ...
            glVertex2d(xPos,
                       static_cast < GLdouble > (waveformDataMonoArray[historyIdx][k]) / 128.0 * heightOfWaveform);
                                        ...
                                        const VisualCamera & aCamera)
{
    ...
    while (count < aVector.size()) {
        ...
        glVertex2d(xCoord,
                   topCoord - diagramCoordHeight + (static_cast < GLdouble >
                                                    (aVector[currIdx] / valueRange * diagramCoordHeight)));
...
void VisualGraphics::drawWaveformSpiral(const uint16 currHistoryNum, const uint16 numberOfWaveformEntries,
...
void VisualGraphics::drawSpectrumAnalyzer(const sint16 currHistoryNum, const uint16 numberOfHistories,
                                          const uint32 numberOfSpectrumEntries, const uint16 numberOfAudioChannels,
                                          const uint8 *** const spectrumDataArray, const VisualCamera & aCamera)
{
    ...
    for (i = 0; i < (numberOfSpectrumEntries / 2); i++) {
        if ((i > 0) && (i % numberOfEntriesPerBarGraph == 0))
            index++;
...
void VisualGraphics::drawSpectrogram(const sint16 currHistoryNum, const uint16 numberOfHistories,
                                     const uint32 numberOfSpectrumEntries, const uint16 numberOfAudioChannels,
                                     const uint8 *** const spectrumDataArray, const VisualCamera & aCamera)
{
...
PixelColor *VisualGraphics::clipPixelData(const PixelColor * inPixelData, const PixelRect & inPixelRect,
                                          const TopLeftPositionedPixelRect & clipRect)
{
    uint8 numberOfBytesPerPixel = 4;
    PixelColor *clippedPixelData =
        (uint32 *) malloc(clipRect.pixelRect.width * clipRect.pixelRect.height * numberOfBytesPerPixel);
...
CGContextRef VisualGraphics::createBitmapContext(size_t pixelWidth, size_t pixelHeight)
{
    ...
    CGContextRef context =
        CGBitmapContextCreate(bitmapData, pixelWidth, pixelHeight, bitsPerComponent, bitmapBytesPerRow, colorSpace,
                              bitmapInfo);
    if (context == NULL) {
        free(bitmapData);
...
bool VisualGraphics::copyARGBBitmapDataToTexture(uint32 textureNumber, uint32 imageWidth, uint32 imageHeight,
                                                 bool canUseRectExtension, const PixelColor ** bitmapData, bool debug)
{
    ...
    if (canUseRectExtension == false) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, potWidth, potHeight, 0, GL_BGRA, GL_UNSIGNED_BYTE,
                     powerOfTwoPixelData);
...
#if TARGET_OS_MAC
        glTexImage2D(GL_TEXTURE_RECTANGLE_EXT, 0, GL_RGBA, imageWidth, imageHeight, 0, GL_BGRA,
                     GL_UNSIGNED_INT_8_8_8_8_REV, argbPixels);
#endif
    }
...
bool VisualGraphics::getCoordsOfPixelPosition(double xPixelPos,
                                              double yPixelPos,
                                              double zAxisPos, double *xCoordPos, double *yCoordPos, double *zCoordPos)
{
    bool success = false;
    ...
    retVal = gluUnProject(xPixelPos,
                          yPixelPos,
                          zAxisPos, modelviewMatrix, projectionMatrix, viewport, xCoordPos, yCoordPos, zCoordPos);
    if (retVal == GL_TRUE) {
        success = true;
    }
    return success;
...
bool VisualGraphics::getPixelsOfCoordPosition(double xCoordPos,
                                              double yCoordPos,
                                              double zCoordPos, double *xPixelPos, double *yPixelPos, double *zAxisPos)
{
    bool success = false;
    ...
    retVal = gluProject(xCoordPos,
                        yCoordPos,
                        zCoordPos, modelviewMatrix, projectionMatrix, viewport, xPixelPos, yPixelPos, zAxisPos);
    if (retVal == GL_TRUE) {
        success = true;
    }
    return success;
...
void VisualGraphics::gatherOpenGLCapabilities()
{
    ...
    // get caps
#if TARGET_OS_MAC
    glGetIntegerv(GL_MAX_TEXTURE_UNITS, (GLint *) & this->capabilities.textureUnits);
    glGetIntegerv(GL_MAX_3D_TEXTURE_SIZE, (GLint *) & this->capabilities.max3DTextureSize);
    glGetIntegerv(GL_MAX_CUBE_MAP_TEXTURE_SIZE, (GLint *) & this->capabilities.maxCubeMapTextureSize);

    // get functionality info
    this->capabilities.fSpecularVector = gluCheckExtension((GLubyte *) "GL_APPLE_specular_vector", strExt);
    this->capabilities.fTransformHint = gluCheckExtension((GLubyte *) "GL_APPLE_transform_hint", strExt);
    this->capabilities.fPackedPixels =
        gluCheckExtension((GLubyte *) "GL_APPLE_packed_pixels", strExt) ||
        gluCheckExtension((GLubyte *) "GL_APPLE_packed_pixel", strExt) || (this->capabilities.glVersion >= 0x0120);
    this->capabilities.fClientStorage = gluCheckExtension((GLubyte *) "GL_APPLE_client_storage", strExt);
    this->capabilities.fYCbCr = gluCheckExtension((GLubyte *) "GL_APPLE_ycbcr_422", strExt);
    this->capabilities.fTextureRange = gluCheckExtension((GLubyte *) "GL_APPLE_texture_range", strExt);
    this->capabilities.fFence = gluCheckExtension((GLubyte *) "GL_APPLE_fence", strExt);
    this->capabilities.fVAR = gluCheckExtension((GLubyte *) "GL_APPLE_vertex_array_range", strExt);
    this->capabilities.fVAO = gluCheckExtension((GLubyte *) "GL_APPLE_vertex_array_object", strExt);
    this->capabilities.fElementArray = gluCheckExtension((GLubyte *) "GL_APPLE_element_array", strExt);
    this->capabilities.fVPEvals = gluCheckExtension((GLubyte *) "GL_APPLE_vertex_program_evaluators", strExt);
    this->capabilities.fFloatPixels = gluCheckExtension((GLubyte *) "GL_APPLE_float_pixels", strExt);
    this->capabilities.fFlushRenderer = gluCheckExtension((GLubyte *) "GL_APPLE_flush_render", strExt);
    this->capabilities.fPixelBuffer = gluCheckExtension((GLubyte *) "GL_APPLE_pixel_buffer", strExt);
    this->capabilities.fImaging = gluCheckExtension((GLubyte *) "GL_ARB_imaging", strExt);
    this->capabilities.fTransposeMatrix =
        gluCheckExtension((GLubyte *) "GL_ARB_transpose_matrix", strExt) || (this->capabilities.glVersion >= 0x0130);
    this->capabilities.fMultitexture =
        gluCheckExtension((GLubyte *) "GL_ARB_multitexture", strExt) || (this->capabilities.glVersion >= 0x0130);
    this->capabilities.fTexEnvAdd =
        gluCheckExtension((GLubyte *) "GL_ARB_texture_env_add", strExt) ||
        gluCheckExtension((GLubyte *) "GL_EXT_texture_env_add", strExt) || (this->capabilities.glVersion >= 0x0130);
    this->capabilities.fTexEnvCombine =
        gluCheckExtension((GLubyte *) "GL_ARB_texture_env_combine", strExt) || (this->capabilities.glVersion >= 0x0130);
    this->capabilities.fTexEnvDot3 =
        gluCheckExtension((GLubyte *) "GL_ARB_texture_env_dot3", strExt) || (this->capabilities.glVersion >= 0x0130);
    ...
    this->capabilities.fTexCubeMap =
        gluCheckExtension((GLubyte *) "GL_ARB_texture_cube_map", strExt) || (this->capabilities.glVersion >= 0x0130);
    this->capabilities.fTexCompress =
        gluCheckExtension((GLubyte *) "GL_ARB_texture_compression", strExt) || (this->capabilities.glVersion >= 0x0130);
    this->capabilities.fMultisample =
        gluCheckExtension((GLubyte *) "GL_ARB_multisample", strExt) || (this->capabilities.glVersion >= 0x0130);
    ...
    this->capabilities.fPointParam =
        gluCheckExtension((GLubyte *) "GL_ARB_point_parameters", strExt) || (this->capabilities.glVersion >= 0x0140);
    this->capabilities.fVertexProg = gluCheckExtension((GLubyte *) "GL_ARB_vertex_program", strExt);
    this->capabilities.fFragmentProg = gluCheckExtension((GLubyte *) "GL_ARB_fragment_program", strExt);
    ...
    this->capabilities.fDepthTex =
        gluCheckExtension((GLubyte *) "GL_ARB_depth_texture", strExt) || (this->capabilities.glVersion >= 0x0140);
    this->capabilities.fShadow =
        gluCheckExtension((GLubyte *) "GL_ARB_shadow", strExt) || (this->capabilities.glVersion >= 0x0140);
    this->capabilities.fShadowAmbient = gluCheckExtension((GLubyte *) "GL_ARB_shadow_ambient", strExt);
    this->capabilities.fVertexBlend = gluCheckExtension((GLubyte *) "GL_ARB_vertex_blend", strExt);
    this->capabilities.fWindowPos =
        gluCheckExtension((GLubyte *) "GL_ARB_window_pos", strExt) || (this->capabilities.glVersion >= 0x0140);
    this->capabilities.fTex3D =
        gluCheckExtension((GLubyte *) "GL_EXT_texture3D", strExt) || (this->capabilities.glVersion >= 0x0120);
    this->capabilities.fClipVolHint = gluCheckExtension((GLubyte *) "GL_EXT_clip_volume_hint", strExt);
    this->capabilities.fRescaleNorm =
        gluCheckExtension((GLubyte *) "GL_EXT_rescale_normal", strExt) || (this->capabilities.glVersion >= 0x0120);
    ...
    this->capabilities.fBlendSub =
        ...
        gluCheckExtension((GLubyte *) "GL_ARB_imaging", strExt);
    this->capabilities.fCVA = gluCheckExtension((GLubyte *) "GL_EXT_compiled_vertex_array", strExt);
    this->capabilities.fTexLODBias =
        gluCheckExtension((GLubyte *) "GL_EXT_texture_lod_bias", strExt) || (this->capabilities.glVersion >= 0x0140);
    this->capabilities.fABGR = gluCheckExtension((GLubyte *) "GL_EXT_abgr", strExt);
    this->capabilities.fBGRA =
        gluCheckExtension((GLubyte *) "GL_EXT_bgra", strExt) || (this->capabilities.glVersion >= 0x0120);
    this->capabilities.fTexFilterAniso = gluCheckExtension((GLubyte *) "GL_EXT_texture_filter_anisotropic", strExt);
    this->capabilities.fPaletteTex = gluCheckExtension((GLubyte *) "GL_EXT_paletted_texture", strExt);
    this->capabilities.fShareTexPalette = gluCheckExtension((GLubyte *) "GL_EXT_shared_texture_palette", strExt);
    this->capabilities.fSecColor =
        gluCheckExtension((GLubyte *) "GL_EXT_secondary_color", strExt) || (this->capabilities.glVersion >= 0x0140);
    this->capabilities.fTexCompressS3TC = gluCheckExtension((GLubyte *) "GL_EXT_texture_compression_s3tc", strExt);
    this->capabilities.fTexRect = gluCheckExtension((GLubyte *) "GL_EXT_texture_rectangle", strExt);
    this->capabilities.fFogCoord = gluCheckExtension((GLubyte *) "GL_EXT_fog_coord", strExt);
    this->capabilities.fDrawRangeElements = gluCheckExtension((GLubyte *) "GL_EXT_draw_range_elements", strExt);
    this->capabilities.fStencilWrap =
        gluCheckExtension((GLubyte *) "GL_EXT_stencil_wrap", strExt) || (this->capabilities.glVersion >= 0x0140);
    this->capabilities.fBlendFuncSep =
        gluCheckExtension((GLubyte *) "GL_EXT_blend_func_separate", strExt) || (this->capabilities.glVersion >= 0x0140);
    this->capabilities.fMultiDrawArrays =
        gluCheckExtension((GLubyte *) "GL_EXT_multi_draw_arrays", strExt) || (this->capabilities.glVersion >= 0x0140);
    this->capabilities.fShadowFunc = gluCheckExtension((GLubyte *) "GL_EXT_shadow_funcs", strExt);
    this->capabilities.fStencil2Side =
        gluCheckExtension((GLubyte *) "GL_EXT_stencil_two_side", strExt) || (this->capabilities.glVersion >= 0x0140);
    ...
    this->capabilities.fTexEdgeClamp =
        gluCheckExtension((GLubyte *) "GL_SGIS_texture_edge_clamp", strExt) || (this->capabilities.glVersion >= 0x0120);
    this->capabilities.fGenMipmap = gluCheckExtension((GLubyte *) "GL_SGIS_generate_mipmap", strExt);
    this->capabilities.fTexLOD =
        gluCheckExtension((GLubyte *) "GL_SGIS_texture_lod", strExt) || (this->capabilities.glVersion >= 0x0120);
    this->capabilities.fPointCull = gluCheckExtension((GLubyte *) "GL_ATI_point_cull_mode", strExt);
    this->capabilities.fTexMirrorOnce = gluCheckExtension((GLubyte *) "GL_ATI_texture_mirror_once", strExt);
    ...
    this->capabilities.fTextFragShader = gluCheckExtension((GLubyte *) "GL_ATI_text_fragment_shader", strExt);
    this->capabilities.fBlendEqSep = gluCheckExtension((GLubyte *) "GL_ATI_blend_equation_separate", strExt);
    this->capabilities.fBlendWeightMinMax = gluCheckExtension((GLubyte *) "GL_ATI_blend_weighted_minmax", strExt);
    this->capabilities.fCombine3 = gluCheckExtension((GLubyte *) "GL_ATI_texture_env_combine3", strExt);
    this->capabilities.fSepStencil = gluCheckExtension((GLubyte *) "GL_ATI_separate_stencil", strExt);
    this->capabilities.fArrayRevComps4Byte = gluCheckExtension((GLubyte *) "GL_ATI_array_rev_comps_in_4_bytes", strExt);
    this->capabilities.fPointSprite = gluCheckExtension((GLubyte *) "GL_NV_point_sprite", strExt);
    this->capabilities.fRegCombiners = gluCheckExtension((GLubyte *) "GL_NV_register_combiners", strExt);
    this->capabilities.fRegCombiners2 = gluCheckExtension((GLubyte *) "GL_NV_register_combiners2", strExt);
    this->capabilities.fTexEnvCombine4 = gluCheckExtension((GLubyte *) "GL_NV_texture_env_combine4", strExt);
    this->capabilities.fBlendSquare =
        gluCheckExtension((GLubyte *) "GL_NV_blend_square", strExt) || (this->capabilities.glVersion >= 0x0140);
    this->capabilities.fFogDist = gluCheckExtension((GLubyte *) "GL_NV_fog_distance", strExt);
    this->capabilities.fMultisampleFilterHint = gluCheckExtension((GLubyte *) "GL_NV_multisample_filter_hint", strExt);
    this->capabilities.fTexGenReflect = gluCheckExtension((GLubyte *) "GL_NV_texgen_reflection", strExt);
    this->capabilities.fTexShader = gluCheckExtension((GLubyte *) "GL_NV_texture_shader", strExt);
    this->capabilities.fTexShader2 = gluCheckExtension((GLubyte *) "GL_NV_texture_shader2", strExt);
    this->capabilities.fTexShader3 = gluCheckExtension((GLubyte *) "GL_NV_texture_shader3", strExt);
    this->capabilities.fDepthClamp = gluCheckExtension((GLubyte *) "GL_NV_depth_clamp", strExt);
    this->capabilities.fLightMaxExp = gluCheckExtension((GLubyte *) "GL_NV_light_max_exponent", strExt);
    this->capabilities.fRasterPosClip = gluCheckExtension((GLubyte *) "GL_IBM_rasterpos_clip", strExt);
...
VisualNurbs *VisualGraphics::createNurbsObject(const VisualItemIdentifier & identifier, uint8 sNumKnots, float *sKnots,
                                               uint8 tNumKnots, float *tKnots, uint16 sStride, uint16 tStride,
                                               uint16 sOrder, uint16 tOrder)
{
...
void VisualGraphics::copyFramebufferToTexture(uint32 textureNumber, bool canUseRectExtension,
                                              const BottomLeftPositionedPixelRect & clipRect, uint16 pixelFormat,
                                              uint16 dataType)
{
    ...
    if (debug == false) {
        if (canUseRectExtension == false) {
            glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (bottomLeftViewportOrigin.x + clipRect.bottomLeftPixel.x),
                             (bottomLeftViewportOrigin.y + clipRect.bottomLeftPixel.y), clipRect.pixelRect.width,
                             clipRect.pixelRect.height, 0);
        } else {
#if TARGET_OS_MAC
            glCopyTexImage2D(GL_TEXTURE_RECTANGLE_EXT, 0, GL_RGBA,
                             (bottomLeftViewportOrigin.x + clipRect.bottomLeftPixel.x),
                             (bottomLeftViewportOrigin.y + clipRect.bottomLeftPixel.y), clipRect.pixelRect.width,
                             clipRect.pixelRect.height, 0);
#endif
        }
        ...
        if (canUseRectExtension == false) {
#if TARGET_OS_WIN
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, clipRect.pixelRect.width, clipRect.pixelRect.height, 0, pixelFormat,
                         dataType, (GLubyte *) image);
#endif
#if TARGET_OS_MAC
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, clipRect.pixelRect.width, clipRect.pixelRect.height, 0, pixelFormat,
                         dataType, (GLuint *) image);
#endif
        } else {
#if TARGET_OS_MAC
            glTexImage2D(GL_TEXTURE_RECTANGLE_EXT, 0, GL_RGBA, clipRect.pixelRect.width, clipRect.pixelRect.height, 0,
                         pixelFormat, dataType, (GLuint *) image);
#endif
        }
        free(image);
    }
