...
VisualNotificationQueue *VisualNotificationQueue::getInstance()
{
    if (VisualNotificationQueue::visualNotificationQueueInstance == NULL) {
#if TARGET_OS_MAC
        MPCriticalRegionID visualNotificationQueueCriticalRegion =
            VisualThreadingManager::getCriticalRegionID(VisualThreadingManager::notificationQueueSingletonMutex);
...
#endif
#if TARGET_OS_WIN
        LPCRITICAL_SECTION visualNotificationQueueCriticalSection =
            VisualThreadingManager::getCriticalSectionPointer(VisualThreadingManager::notificationQueueSingletonMutex);
        ...
        if (VisualNotificationQueue::visualNotificationQueueInstance == NULL) {
            ...
            if (VisualNotificationQueue::visualNotificationQueueInstance == NULL) {
                char errLog[256];
                sprintf(errLog, "ERR: init VisualNotificationQueue failed in file: %s (line: %d) [%s])", __FILE__,
...
void VisualNotificationQueue::push(VisualNotification & aNotification)
{
    ...
    MPCriticalRegionID inputNotificationQueueAccessCriticalRegion =
        VisualThreadingManager::getCriticalRegionID(VisualThreadingManager::inputNotificationQueueAccessMutex);
    OSStatus status = noErr;
...
#endif
#if TARGET_OS_WIN
    LPCRITICAL_SECTION inputNotificationQueueAccessCriticalSection =
        VisualThreadingManager::getCriticalSectionPointer(VisualThreadingManager::inputNotificationQueueAccessMutex);
...
#if TARGET_OS_MAC
    MPCriticalRegionID visualNotificationQueueAccessCriticalRegion =
        VisualThreadingManager::getCriticalRegionID(VisualThreadingManager::notificationQueueAccessMutex);
    OSStatus status = noErr;
...
#endif
#if TARGET_OS_WIN
    LPCRITICAL_SECTION visualNotificationQueueAccessCriticalSection =
        VisualThreadingManager::getCriticalSectionPointer(VisualThreadingManager::notificationQueueAccessMutex);
...
#endif
#if TARGET_OS_WIN
    LPCRITICAL_SECTION notificationQueueAccessCriticalSection =
        VisualThreadingManager::getCriticalSectionPointer(VisualThreadingManager::notificationQueueAccessMutex);
    ::EnterCriticalSection(notificationQueueAccessCriticalSection);
#endif

    for (NotificationQueueIterator it = theVisualNotificationQueue->inputNotificationQueue.begin();
         it != theVisualNotificationQueue->inputNotificationQueue.end(); it++) {
