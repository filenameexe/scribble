...
class RovastarDarkSecret:public Pipeline {
  public:
    ...
    virtual void Render(const BeatDetect & music, const PipelineContext & context) {
        ...
        float val =
            1.2 * music.bass + 0.4 * music.bass_att + 0.1 * music.treb + 0.1 * music.treb_att + 0.1 * music.mid +
            0.1 * music.mid_att;

        q8 = oldq8 +
            0.003 *
            (((pow(val, 6) / context.fps) + (pow(val, 5) / context.fps) + (pow(val, 4) / context.fps) +
              (pow(val, 3) / context.fps) + (pow(val, 2) / context.fps) + (val) / context.fps));
        oldq8 = q8;
