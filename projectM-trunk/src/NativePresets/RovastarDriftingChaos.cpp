...
class RovastarDriftingChaos:public Pipeline {
  public:
    ...
    virtual void Render(const BeatDetect & music, const PipelineContext & context) {
        ...
        xdir =
            if_milk(above(abs(xpos), xamp), -sign(xpos),
                    if_milk(below(abs(xspeed), 0.1), 2 * above(xpos, 0) - 1, xdir));
        ...
        ydir =
            if_milk(above(abs(ypos), yamp), -sign(ypos),
                    if_milk(below(abs(yspeed), 0.1), 2 * above(ypos, 0) - 1, ydir));
        ...
        q8 = oldq8 +
            0.0003 *
            (powf
             (1 + 1.2 * music.bass + 0.4 * music.bass_att + 0.1 * music.treb + 0.1 * music.treb_att + 0.1 * music.mid +
              0.1 * music.mid_att, 6) / context.fps);
        oldq8 = q8;
        q7 = 0.003 *
            (powf
             (1 + 1.2 * music.bass + 0.4 * music.bass_att + 0.1 * music.treb + 0.1 * music.treb_att + 0.1 * music.mid +
