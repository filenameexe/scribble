...
class PlaylistWriteFunctor {
  public:
    PlaylistWriteFunctor(const QVector < QProjectM_MainWindow::PlaylistItemMetaData * >::iterator & begin,
                         const QVector < QProjectM_MainWindow::PlaylistItemMetaData * >::iterator & end,
                         const QString & desc):m_pos(begin), m_end(end), m_desc(desc) {
...
activePresetIndex(new Nullable < long >), playlistItemCounter(0), m_QPresetEditorDialog(0)
{
    ...
    connect(ui->lockPresetCheckBox, SIGNAL(stateChanged(int)), m_QProjectMWidget, SLOT(setPresetLock(int)));

    connect(ui->shuffleEnabledCheckBox, SIGNAL(stateChanged(int)), m_QProjectMWidget, SLOT(setShuffleEnabled(int)));

    connect(ui->clearPresetList_PushButton, SIGNAL(pressed()), this, SLOT(clearPlaylist()));
    ...
    if (!m_QProjectMWidget->isValid()) {
        int ret = QMessageBox::warning(this, tr("projectM cannot be started."),
                                       tr
                                       ("Your graphics driver or configuration is not supported by projectM. Please contact the developers (carmelo.piccione+projectM@gmail.com or psperl+projectM@gmail.com) with your card and driver information so we can help you get it working."),
                                       QMessageBox::Ok);
        exit(-1);
    }
    ...
    connect(ui->tableView, SIGNAL(activated(const QModelIndex &)), this, SLOT(selectPlaylistItem(const QModelIndex &)));
...
void QProjectM_MainWindow::updatePlaylistSelection(bool hardCut, unsigned int index)
{
    ...
    if (hardCut)
        statusBar()->
            showMessage(tr
                        (QString("*** Hard cut to \"%1\" ***").arg(this->qprojectM()->getPresetName(index).c_str()).
                         toStdString().c_str()), 2000);
    else
        statusBar()->showMessage(tr("*** Soft cut to \"%1\" ***").arg(this->qprojectM()->getPresetName(index).c_str()).
                                 toStdString().c_str(), 2000);
...
void QProjectM_MainWindow::openPresetEditorDialog(int rowIndex)
{
    ...
    if (!m_QPresetEditorDialog) {
        ...
        connect(m_QPresetEditorDialog, SIGNAL(presetModified(int)), this, SLOT(selectPlaylistItem(int)));
    }
...
void QProjectM_MainWindow::addPresetsDialog(const QModelIndex & index)
{
    ...
    if (m_QPresetFileDialog->exec()) {
        ...
        for (QHash < QString, PlaylistItemVector * >::iterator pos = historyHash.begin(); pos != historyHash.end();
             ++pos) {
            if (pos.key() != QString())
                delete(pos.value());
        }
...
void QProjectM_MainWindow::savePlaylist()
{
    ...
    if (playlistModel->writePlaylist(m_currentPlaylistUrl)) {
        this->ui->statusbar->showMessage(QString("Saved cropped preset playlist \"%1\" successfully.").
                                         arg(m_currentPlaylistUrl), 3000);
    ...
    if (!qfile.open(QIODevice::WriteOnly)) {
        QMessageBox::warning(0, "Playlist Save Error",
                             QString
                             ("There was a problem trying to save the playlist \"%1\".  You may not have permission to modify this file.").
                             arg(m_currentPlaylistUrl));
        return;
    }
    ...
    PlaylistWriteFunctor writeFunctor(itemMetaDataVector.begin(), itemMetaDataVector.end(),
                                      playlistModel->playlistDesc());


    QXmlPlaylistHandler::writePlaylist(&qfile, writeFunctor);
    this->ui->statusbar->showMessage(QString("Saved preset playlist \"%1\" successfully.").arg(m_currentPlaylistUrl),
                                     4000);
    this->ui->presetPlayListDockWidget->setWindowModified(false);
...
void QProjectM_MainWindow::updatePlaylistUrl(const QString & url = QString())
{
    ...
    if (url == QString()) {
        ...
        ui->presetPlayListDockWidget->setWindowTitle(QString("Preset Directory - %1 [*]").arg(url));
...
void QProjectM_MainWindow::insertPlaylistItem(const PlaylistItemMetaData & data, int targetIndex) {
