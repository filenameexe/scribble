...
void Shape::Draw(RenderContext & context)
{
    ...
    if (textured) {
        ...
        for (int i = 1; i < sides + 2; i++) {
            ...
            t = (i - 1) / (float) sides;
            tex[i][0] =
                0.5f + 0.5f * cosf(t * 3.1415927f * 2 + tex_ang +
                                   3.1415927f * 0.25f) * (context.aspectCorrect ? context.aspectRatio : 1.0) / tex_zoom;
            tex[i][1] = 0.5f + 0.5f * sinf(t * 3.1415927f * 2 + tex_ang + 3.1415927f * 0.25f) / tex_zoom;
            points[i][0] =
                temp_radius * cosf(t * 3.1415927f * 2 + ang +
                                   3.1415927f * 0.25f) * (context.aspectCorrect ? context.aspectRatio : 1.0) + xval;
...
void Border::Draw(RenderContext & context)
{
    ...
    float pointsH[4][2] =
        { {of + iff, texof}, {of + iff, texof - iff}, {texof - iff, texof}, {texof - iff, texof - iff} };
