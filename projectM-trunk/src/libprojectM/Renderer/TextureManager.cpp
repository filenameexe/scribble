...
void TextureManager::Preload()
{
...
#else
    unsigned int tex = SOIL_load_OGL_texture_from_memory(M_data,
                                                         M_bytes,
                                                         SOIL_LOAD_AUTO,
                                                         SOIL_CREATE_NEW_ID,
...
#else
    tex = SOIL_load_OGL_texture_from_memory(project_data,
                                            project_bytes,
                                            SOIL_LOAD_AUTO,
                                            SOIL_CREATE_NEW_ID, SOIL_FLAG_POWER_OF_TWO | SOIL_FLAG_MULTIPLY_ALPHA
                                            //|  SOIL_FLAG_COMPRESS_TO_DXT
        );
#endif
    ...
    tex = ilutGLBindTexImage();
#else
    tex = SOIL_load_OGL_texture_from_memory(headphones_data,
                                            headphones_bytes,
                                            SOIL_LOAD_AUTO,
                                            SOIL_CREATE_NEW_ID, SOIL_FLAG_POWER_OF_TWO | SOIL_FLAG_MULTIPLY_ALPHA
                                            // |  SOIL_FLAG_COMPRESS_TO_DXT
...
GLuint TextureManager::getTextureFullpath(const std::string filename, const std::string imageURL)
{
    ...
    if (textures.find(filename) != textures.end()) {
        ...
        unsigned int tex = SOIL_load_OGL_texture_size(imageURL.c_str(),
                                                      SOIL_LOAD_AUTO,
                                                      SOIL_CREATE_NEW_ID,
