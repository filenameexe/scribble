...
{
    for (std::vector < PresetFactory * >::iterator pos = _factoryList.begin(); pos != _factoryList.end(); ++pos) {
        assert(*pos);
        delete(*pos);
    }
...
bool PresetFactoryManager::extensionHandled(const std::string & extension) const const
{
