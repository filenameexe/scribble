...
projectM::projectM(std::string config_file, int flags):beatDetect(0), renderer(0), _pcm(0), m_presetPos(0),
m_flags(flags), _pipelineContext(new PipelineContext()), _pipelineContext2(new PipelineContext())
{
...
projectM::projectM(Settings settings, int flags):beatDetect(0), renderer(0), _pcm(0), m_presetPos(0), m_flags(flags),
...
/*
    ...
    if (timeKeeper->IsSmoothing() && timeKeeper->SmoothRatio() <= 1.0 && !m_presetChooser->empty()) {
        ...
        PipelineMerger::mergePipelines(m_activePreset->pipeline(),
                                       ...
                                       _matcher->matchResults(), *_merger, timeKeeper->SmoothRatio());
...
void projectM::projectM_init(int gx, int gy, int fps, int texsize, int width, int height)
{
    ...
    this->renderer =
        new Renderer(width, height, gx, gy, texsize, beatDetect, settings().presetURL, settings().titleFontURL,
    ...
    if (pthread_create(&thread, NULL, thread_callback, this) != 0) {
        ...
        std::cerr << "[projectM] failed to allocate a thread! try building with option USE_THREADS turned off" << std::
            endl;;
        exit(EXIT_FAILURE);
    }
    pthread_mutex_lock(&mutex);
...
bool projectM::isPresetLocked() const const
{
    return renderer->noSwitch;
}

std::string projectM::getPresetURL(unsigned int index) const const
{
    return m_presetLoader->getPresetURL(index);
}

int projectM::getPresetRating(unsigned int index, const PresetRatingType ratingType) const const
{
    return m_presetLoader->getPresetRating(index, ratingType);
}

std::string projectM::getPresetName(unsigned int index) const const
{
...
bool projectM::selectedPresetIndex(unsigned int &index) const const
{
...
bool projectM::presetPositionValid() const const
{

    return (*m_presetPos != m_presetChooser->end());
}

unsigned int projectM::getPlaylistSize() const const
{
