...
FTSize::FTSize()
:  ftFace(0), ftSize(0), size(0), xResolution(0), yResolution(0), err(0)
{
}
...
unsigned int FTSize::CharSize() const const
{
    return size;
}


float FTSize::Ascender() const const
{
    return ftSize == 0 ? 0.0f : static_cast < float >(ftSize->metrics.ascender) / 64.0f;
}


float FTSize::Descender() const const
{
    return ftSize == 0 ? 0.0f : static_cast < float >(ftSize->metrics.descender) / 64.0f;
}


float FTSize::Height() const const
{
    if (0 == ftSize) {
        return 0.0f;
    }
...
float FTSize::Width() const const
{
    if (0 == ftSize) {
        return 0.0f;
    }
    ...
    if (FT_IS_SCALABLE((*ftFace))) {
        return ((*ftFace)->bbox.xMax - (*ftFace)->bbox.xMin) * (static_cast <
                                                                float >(ftSize->metrics.x_ppem) / static_cast <
...
float FTSize::Underline() const const
{
    return 0.0f;
}
