...
QPulseAudioThread::QPulseAudioThread(int _argc, char **_argv, QProjectM_MainWindow * mainWindow):QThread(0),
argc(_argc), argv(_argv), m_qprojectM_MainWindow(mainWindow)
{
...
void QPulseAudioThread::reconnect(SourceContainer::const_iterator pos = s_sourceList.end())
{
    ...
    pa_stream_set_state_callback(stream, stream_state_callback, &s_sourceList);
...
void QPulseAudioThread::cork()
{
    ...
    pa_operation *op = pa_stream_cork(stream, b, pa_stream_success_callback, this);
...
void QPulseAudioThread::stream_moved_callback(pa_stream * s, void *userdata)
{
    ...
    if (verbose)
        fprintf(stderr, "Stream moved to device %s (%u, %ssuspended).\n", pa_stream_get_device_name(s),
                pa_stream_get_device_index(s), pa_stream_is_suspended(s) ? "" : "not ");
}
...
void QPulseAudioThread::stdout_callback(pa_mainloop_api * a, pa_io_event * e, int fd, pa_io_event_flags_t f,
                                        void *userdata)
{
    ...
    if (!buffer) {
        ...
        qprojectM_MainWindow->addPCM(buffer + buffer_index, buffer_length / (sizeof(float)));
        //s_audioMutex->unlock();
...
void QPulseAudioThread::stream_update_timing_callback(pa_stream * s, int success, void *userdata)
{
    ...
    if (!success || pa_stream_get_time(s, &usec) < 0 || pa_stream_get_latency(s, &latency, &negative) < 0) {
    ...
    fprintf(stderr, "Time: %0.3f sec; Latency: %0.0f usec.  \r",
            (float) usec / 1000000, (float) latency * (negative ? -1 : 1));
}
...
void QPulseAudioThread::subscribe_callback(struct pa_context *c, enum pa_subscription_event_type t, uint32_t index,
                                           void *userdata)
{
    ...
    case PA_SUBSCRIPTION_EVENT_SOURCE:
        {
            if ((t & PA_SUBSCRIPTION_EVENT_TYPE_MASK) == PA_SUBSCRIPTION_EVENT_REMOVE) {
                ...
                pa_operation_unref(pa_context_get_source_info_by_index
                                   (context, index, pa_source_info_callback, userdata));
            }
            break;
        }
...
void QPulseAudioThread::time_event_callback(pa_mainloop_api * m, pa_time_event * e, const struct timeval *tv,
                                            void *userdata)
{
    struct timeval next;
...
{
    pa_operation *op;
    pa_operation_unref(pa_context_get_source_info_list(context, pa_source_info_callback, pulseThread));
