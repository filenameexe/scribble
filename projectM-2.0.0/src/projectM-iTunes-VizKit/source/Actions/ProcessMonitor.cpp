...
void ProcessMonitor::showAudioInfo(const UInt32 elapsedAudioTime, const UInt32 remainingAudioTime)
{
    ...
    VisualActorGraphics::resetModelViewMatrix();
    VisualActorGraphics::drawSpectrogram(currMusicDataHistory, maxNumberOfMusicDataHistories, numberOfSpectrumEntries,
                                         numberOfAudioDataChannels, spectrumDataArray);
    VisualActorGraphics::resetModelViewMatrix();
    ...
    VisualActorGraphics::drawTrackProgressMeter(&this->progressMeterBackgroundVertices, &this->progressMeterVertices,
                                                &this->progressMeterOutlineVertices);

}


void ProcessMonitor::registerProcessMonitorInfoMap(const std::map < std::string,
...
void ProcessMonitor::calcTrackProgressMeterVertices(const UInt32 elapsedAudioTime, const UInt32 remainingAudioTime)
{
    ...
    rightProgressPos =
        VisualActorGraphics::getMaxLeftCoordOfCanvas() +
        ((VisualActorGraphics::getMaxRightCoordOfCanvas() -
          VisualActorGraphics::getMaxLeftCoordOfCanvas()) * ((float) elapsedAudioTime / (float) (remainingAudioTime +
                                                                                                 elapsedAudioTime)));
