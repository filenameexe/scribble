...
void VisualDataStore::setDefaultPreferenceValueInt(const VisualConfiguration::PreferenceKey anIdentifier,
                                                   const int aValue)
{
    bool found = false;
...
void VisualDataStore::setDefaultPreferenceValueFloat(const VisualConfiguration::PreferenceKey anIdentifier,
                                                     const float aValue)
{
    bool found = false;
    Preference *aPreference;
...
void VisualDataStore::setDefaultPreferenceValueChar(const VisualConfiguration::PreferenceKey anIdentifier,
...
void VisualDataStore::setPreferenceValueChar(const VisualConfiguration::PreferenceKey anIdentifier,
                                             const char *const aValue)
{
    bool found = false;
    Preference *aPreference;
...
int VisualDataStore::getValueInt(const VisualConfiguration::PreferenceKey anIdentifier, bool * wasSet)
{
    ...
    theVisualDataStore = VisualDataStore::getInstance();
    for (PreferenceVectorIterator it = theVisualDataStore->preferenceVector.begin();
         it != theVisualDataStore->preferenceVector.end(); it++) {
        if ((*it)->key == anIdentifier && (*it)->dataType == kIntPrefType) {
...
void VisualDataStore::getValueChar(const VisualConfiguration::PreferenceKey anIdentifier, char *outPrefVal,
                                   bool * wasSet)
{
...
int VisualDataStore::getDefaultPreferenceValueInt(const VisualConfiguration::PreferenceKey anIdentifier,
                                                  bool * isDefaultValue)
{
    char errLog[128];
...
float VisualDataStore::getDefaultPreferenceValueFloat(const VisualConfiguration::PreferenceKey anIdentifier,
                                                      bool * isDefaultValue)
{
    char errLog[128];
    *isDefaultValue = false;
...
bool VisualDataStore::getDefaultPreferenceValueChar(const VisualConfiguration::PreferenceKey anIdentifier,
                                                    char *outPrefVal)
{
    char errLog[128];
...
int VisualDataStore::getPreferenceValueInt(const VisualConfiguration::PreferenceKey anIdentifier, bool * isDefaultValue)
{
    ...
    CFStringRef appID =
        CFStringCreateWithCString(kCFAllocatorDefault, VisualConfiguration::kVisualPluginDomainIdentifier,
                                  kCFStringEncodingISOLatin1);
    intRef =
        (CFNumberRef) CFPreferencesCopyValue(identifierStr, appID, kCFPreferencesCurrentUser, kCFPreferencesAnyHost);
    CFRelease(appID);
    if (intRef) {
    ...
    sprintf(prefPath, "\\%s\\%s", VisualConfiguration::kVisualPluginDomainIdentifier,
            VisualConfiguration::kVisualPluginName);
...
float VisualDataStore::getPreferenceValueFloat(const VisualConfiguration::PreferenceKey anIdentifier,
                                               bool * isDefaultValue)
{
...
void VisualDataStore::getPreferenceValueChar(const VisualConfiguration::PreferenceKey anIdentifier, char *outPrefVal,
                                             bool * isDefaultValue)
{
    ...
    CFStringRef appID =
        CFStringCreateWithCString(kCFAllocatorDefault, VisualConfiguration::kVisualPluginDomainIdentifier,
                                  kCFStringEncodingISOLatin1);
    stringRef =
        (CFStringRef) CFPreferencesCopyValue(identifierStr, appID, kCFPreferencesCurrentUser, kCFPreferencesAnyHost);
    CFRelease(appID);
    if (stringRef) {
...
void VisualDataStore::storePreferences()
{
    ...
    for (it = theVisualDataStore->preferenceVector.begin(); it != theVisualDataStore->preferenceVector.end(); it++) {
        ...
        if ((*it)->status == kPrefSetButNotStored) {
            ...
            identifierStr =
                CFStringCreateWithCString(kCFAllocatorDefault, preferenceKeyStr, kCFStringEncodingISOLatin1);
            if ((*it)->dataType == kCharPrefType) {
                prefString =
                    CFStringCreateWithCString(kCFAllocatorDefault, (*it)->valueChar, kCFStringEncodingISOLatin1);
                CFPreferencesSetValue(identifierStr, prefString, appID, kCFPreferencesCurrentUser,
        ...
        if (numberOfBytesRead > 0) {
            ...
            while (token != NULL) {
                ...
                if (valueStr) {
                    ...
                    found = false;
                    for (it = theVisualDataStore->preferenceVector.begin();
                         it != theVisualDataStore->preferenceVector.end(); it++) {
                        char currKeyStr[128];
...
bool VisualDataStore::storeAudioTrackMetadata(const ITTrackInfo * const trackMetadata)
{
    ...
    if (*currAudioMetaData !=
        *theVisualDataStore->audioMetaDataHistory[theVisualDataStore->currAudioMetaDataHistoryIdx]) {
...
bool VisualDataStore::storeAudioStreamMetadata(const ITStreamInfo * const streamMetadata)
{
    ...
    if (streamMetadata->streamTitle[0] > 0) {
        VisualString streamTitle((const UniChar *) streamMetadata->streamTitle + 1,
                                 (UInt32) streamMetadata->streamTitle[0]);
        currAudioMetaData->setTrackName(streamTitle);
    } else if (streamMetadata->streamMessage[0] > 0) {
        VisualString streamMessage((const UniChar *) streamMetadata->streamMessage + 1,
                                   (UInt32) streamMetadata->streamMessage[0]);
...
{
    theVisualDataStore = VisualDataStore::getInstance();
    return theVisualDataStore->audioMetaDataHistory[theVisualDataStore->currAudioMetaDataHistoryIdx]->
        getTrackSizeInBytes();
}
...
UInt16 VisualDataStore::evaluateCoverArtInfo(OSType * albumCoverArtworkFileType, Handle * albumCoverArtworkHandle)
{
    ...
    numberOfArtworks =
        VisualHostCommunication::getCurrentTrackCoverArt(albumCoverArtworkFileType, albumCoverArtworkHandle);
...
void VisualDataStore::disposeHandleOfCoverArt()
{
    theVisualDataStore = VisualDataStore::getInstance();
    theVisualDataStore->audioMetaDataHistory[theVisualDataStore->currAudioMetaDataHistoryIdx]->
        disposeAlbumCoverArtworkHandle();
}


void VisualDataStore::setPreferredDisplayResolution(UInt16 minBitsPerPixel, UInt16 maxBitsPerPixel,
                                                    ...
                                                    UInt16 verticalPixels)
{
    ...
    if (horizontalPixels > 0 && verticalPixels > 0) {
        status =
            VisualHostCommunication::setPreferredDisplayResolution(minBitsPerPixel, maxBitsPerPixel,
                                                                   preferredBitsPerPixel, horizontalPixels,
                                                                   verticalPixels);
    } else {
        status = noErr;
    }
...
{
    char *Path = (char *) iPath, *TmpPath = Path, TmpSmb = 0, *LastDPtr = NULL;
...
{
    if (VisualDataStore::lyricsCreationThreadIsRunning == true)
        return;
    VisualThreading::createThread((ThreadingFuncPtr) VisualDataStore::createLyricsOfCurrentTrackThread,
                                  "createLyricsOfCurrentTrackThread");
}
