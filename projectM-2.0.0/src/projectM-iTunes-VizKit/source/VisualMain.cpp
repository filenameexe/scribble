...
OSStatus VisualMain::renderAction()
{
        ...
        //VisualStageControl::checkForFadeOutEvent();
            ...
            sprintf(logStr, "Notification (%d/%d): %s", currNotificationIdx, (UInt16) notificationCount,
                     notificationStr);
            ...
            sprintf(errLog, "unhandled VisualNotificationKey %s in file: %s (line: %d) [%s])", notificationString,
                     __FILE__, __LINE__, __FUNCTION__);
            writeLog(errLog);
    } } AudioPlayState playState = theVisualPlayerState->getAudioPlayState();
    if (playState == kAudioPlayStarted) {
        UInt32 fadeInTimeOnPlayInMS =
            VisualDataStore::getPreferenceValueInt(VisualConfiguration::kFadeInTimeOnPlayInMS);
        ...
        UInt32 fadeInTimeOnResumeInMS =
            VisualDataStore::getPreferenceValueInt(VisualConfiguration::kFadeInTimeOnResumeInMS);
        ...
        globalPM->projectM_resetGL(theVisualGraphics->getCanvasPixelWidth(),
                                    theVisualGraphics->getCanvasPixelHeight());
...
void VisualMain::handleAudioPlayStoppedEvent()
{
    ...
    if (theVisualPlayerState->remainingAudioTimeIsKnown() == true
          && theVisualPlayerState->getRemainingAudioTime() > 2000) {
...
//      VisualAudioLab* theVisualAudioLab = NULL;
        ...
        //visualPluginData = (VisualPluginData*) refCon;
             ...
             */ 
            VisualDataStore::setAppVersionNum(messageInfo->u.initMessage.appVersion.majorRev,
                                               messageInfo->u.initMessage.appVersion.minorAndBugRev);
...
//                      VisualAudioLab::setNumberOfAudioSpectrumDataEntries(kVisualNumSpectrumEntries);
    ...
    case kVisualPluginDisableMessage:
             ...
             */ 
                ...
                VisualDataStore::setPreferredDisplayResolution(minBitsPerPixel, maxBitsPerPixel,
                                                                thePreferredDisplayResolution.bitsPerPixel,
                                                                thePreferredDisplayResolution.horizontalPixels,
                                                                thePreferredDisplayResolution.verticalPixels);
            ...
            isSetupForFullScreenMode((bool)
                                     ((messageInfo->u.showWindowMessage.options & kWindowIsFullScreen) ==
                                      kWindowIsFullScreen));
            ...
            handleShowShouldStartMessage((bool)
                                         ((messageInfo->u.showWindowMessage.options & kWindowIsFullScreen) ==
                                          kWindowIsFullScreen));
            ...
            isSetupForFullScreenMode((bool)
                                     ((messageInfo->u.setWindowMessage.options & kWindowIsFullScreen) ==
                                      kWindowIsFullScreen));
...
*/ 
             ...
             */ 
               ...
               || messageInfo->u.changeTrackMessage.streamInfoUnicode->streamURL[0] > 0) {
                ...
                VisualDataStore::storeAudioStreamMetadata(messageInfo->u.changeTrackMessage.streamInfoUnicode);
                ...
                VisualDataStore::storeAudioTrackMetadata(messageInfo->u.changeTrackMessage.trackInfoUnicode);
...
OSStatus VisualMain::RegisterVisualPlugin(PluginMessageInfo * messageInfo)
{
        ...
        (UInt8) strlen(VisualConfiguration::kVisualPluginShowName);
    memcpy(&playerMessageInfo.u.registerVisualPluginMessage.name[1], VisualConfiguration::kVisualPluginShowName,
            strlen(VisualConfiguration::kVisualPluginShowName));
    SetNumVersion(&playerMessageInfo.u.registerVisualPluginMessage.pluginVersion, gVisualPluginMajorVersion,
                    gVisualPluginMinorVersion, gVisualPluginReleaseStage, gVisualPluginNonFinalRelease);
    ...
    playerMessageInfo.u.registerVisualPluginMessage.numWaveformChannels = 2;
        ...
        PlayerRegisterVisualPlugin(messageInfo->u.initMessage.appCookie, messageInfo->u.initMessage.appProc,
                                   &playerMessageInfo);
