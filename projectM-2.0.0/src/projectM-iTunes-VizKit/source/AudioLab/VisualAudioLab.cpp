...
void VisualAudioLab::processWaveformData(const UInt8 numWaveformChannels, const UInt16 numWaveformEntries,
                                         const UInt8 * const currWaveformData)
{
...
void VisualAudioLab::addToBeatGapHistogram(UInt16 gapInMilliseconds)
{
    ...
    if (beatGapHistogramIdx > (MAXNUMBEROFBEATHISTOGRAMBINS - 1))
        return;
...
UInt32 VisualAudioLab::getBestBeatMSInterval()
{
    ...
    if (beatIndex == 0) {
        bestBeatMSInterval =
            ...
            beatsRecordedInBeatAnalyseTimeFrame[MAXNUMBEROFBEATSRECORDEDINANALYSETIMEFRAME].
