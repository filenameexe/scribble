...
void OptionsDialog::show()
{
    ...
    if (theOptionsDialog->optionsDialogWindow == NULL) {
        ...
        // tab control
        GetControlByID(theOptionsDialog->optionsDialogWindow, &theOptionsDialog->tabControlID,
                       &theOptionsDialog->tabControl);

        // display resolution menu pop up control
        err =
            GetControlByID(theOptionsDialog->optionsDialogWindow, &theOptionsDialog->displayResolutionMenuControlID,
                           &(theOptionsDialog->displayResolutionPopUpControl));
        if (err != noErr) {
        ...
        while (theVisualGraphics->getNextAvailableDisplayResolution(str, &isSelected)) {
            ...
            err =
                AppendMenuItemTextWithCFString(theOptionsDialog->displayResolutionMenu, displayResolutionShowStr, 0, 0,
                                               NULL);
            if (err != noErr) {
        ...
        SetControlPopupMenuHandle(theOptionsDialog->displayResolutionPopUpControl,
                                  theOptionsDialog->displayResolutionMenu);
        SetControl32BitMaximum(theOptionsDialog->displayResolutionPopUpControl, count);

        InstallWindowEventHandler(theOptionsDialog->optionsDialogWindow, NewEventHandlerUPP(optionsDialogWindowHandler),
                                  1, &windowCloseEvent, NULL, NULL);
        InstallWindowEventHandler(theOptionsDialog->optionsDialogWindow,
                                  NewEventHandlerUPP(optionsDialogControlHandler), 1, &controlHitEvent, NULL, NULL);
    ...
    SetControl32BitValue(theOptionsDialog->displayResolutionPopUpControl,
                         theOptionsDialog->displayResolutionMenuSelectedIdx);
        ...
        case displayResolutionMenuId:
            {
                ...
                UInt16 bitsPerPixel;
                UInt16 refreshRate;
                theVisualGraphics->matchDisplayResolutionShowStrWithPrefs(str, horizontalPixels, verticalPixels,
                                                                          bitsPerPixel, refreshRate);
                ...
                VisualDataStore::setPreferredDisplayResolution(minBitsPerPixel, maxBitsPerPixel, bitsPerPixel,
