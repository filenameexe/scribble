...
VisualGraphics::~VisualGraphics()
{
    ...
    for (DisplayResolutionVectorIterator it = theVisualGraphics->displayResolutionVector.begin();
         it != theVisualGraphics->displayResolutionVector.end(); it++) {
        delete *it;
        *it = NULL;
    }
...
void VisualGraphics::setOrthographicProjection()
{
    ...
    theVisualGraphicsCore->setViewport(&(theVisualGraphics->canvasRect), &canvasSurroundingRect,
                                       theVisualGraphics->isSetUpOnFullscreen);

    double maxTopPos, maxLeftPos, maxBottomPos, maxRightPos, maxNearPos, maxFarPos;

    if ((theVisualGraphics->canvasRect.right - theVisualGraphics->canvasRect.left) <=
        (theVisualGraphics->canvasRect.bottom - theVisualGraphics->canvasRect.top)) {
        maxLeftPos = -1.0;
        maxRightPos = 1.0;
        maxBottomPos =
            -1.0 * (GLdouble) (theVisualGraphics->canvasRect.bottom -
                               theVisualGraphics->canvasRect.top) / (GLdouble) (theVisualGraphics->canvasRect.right -
                                                                                theVisualGraphics->canvasRect.left);
        maxTopPos =
            1.0 * (GLdouble) (theVisualGraphics->canvasRect.bottom -
                              theVisualGraphics->canvasRect.top) / (GLdouble) (theVisualGraphics->canvasRect.right -
                                                                               theVisualGraphics->canvasRect.left);
        ...
        maxLeftPos =
            -1.0 * (GLdouble) (theVisualGraphics->canvasRect.right -
                               theVisualGraphics->canvasRect.left) / (GLdouble) (theVisualGraphics->canvasRect.bottom -
                                                                                 theVisualGraphics->canvasRect.top);
        maxRightPos =
            1.0 * (GLdouble) (theVisualGraphics->canvasRect.right -
                              theVisualGraphics->canvasRect.left) / (GLdouble) (theVisualGraphics->canvasRect.bottom -
                                                                                theVisualGraphics->canvasRect.top);
...
void VisualGraphics::setPerspectiveProjection(double maxNearCoord)
{
    ...
    if ((theVisualGraphics->canvasRect.right - theVisualGraphics->canvasRect.left) <=
        ...
        maxRightCoord = 1.0;
        maxBottomCoord =
            -1.0 * (GLfloat) (theVisualGraphics->canvasRect.bottom -
                              theVisualGraphics->canvasRect.top) / (GLfloat) (theVisualGraphics->canvasRect.right -
                                                                              theVisualGraphics->canvasRect.left);
        maxTopCoord =
            1.0 * (GLfloat) (theVisualGraphics->canvasRect.bottom -
                             theVisualGraphics->canvasRect.top) / (GLfloat) (theVisualGraphics->canvasRect.right -
        ...
        maxLeftCoord =
            -1.0 * (GLfloat) (theVisualGraphics->canvasRect.right -
                              theVisualGraphics->canvasRect.left) / (GLfloat) (theVisualGraphics->canvasRect.bottom -
                                                                               theVisualGraphics->canvasRect.top);
        maxRightCoord =
            1.0 * (GLfloat) (theVisualGraphics->canvasRect.right -
                             theVisualGraphics->canvasRect.left) / (GLfloat) (theVisualGraphics->canvasRect.bottom -
...
void VisualGraphics::gatherAvailableDisplayResolutions()
{
    ...
    for (it = theVisualGraphics->displayResolutionVector.begin();
         it != theVisualGraphics->displayResolutionVector.end(); it++) {
        delete *it;
        *it = NULL;
    }
    ...
    for (i = 0; i < theDisplayModeCount; i++) {
        ...
        if (displayResolutionIsAccepted == true) {
            if (((refreshRate >= minRefreshRate) || (refreshRate == 0)) && (bitsPerPixel >= minBitsPerPixel)
    ...
    do {
        ...
        if (flag) {
            ...
            if (displayResolutionIsAccepted == true) {
                if ((devmode.dmDisplayFrequency >= minRefreshRate) && (devmode.dmBitsPerPel >= minBitsPerPixel)
                    && (devmode.dmBitsPerPel <= maxBitsPerPixel)) {
                    displayResolutionIsAccepted = true;
                } else {
                    displayResolutionIsAccepted = false;
                }
            ...
            if (displayResolutionIsAccepted == true) {
                for (it = theVisualGraphics->displayResolutionVector.begin();
                     it != theVisualGraphics->displayResolutionVector.end(); it++) {
                    if (((*it)->horizontalPixels == devmode.dmPelsWidth)
                        && ((*it)->verticalPixels == devmode.dmPelsHeight)) {
                        found = true;
    ...
    std::sort(theVisualGraphics->displayResolutionVector.begin(), theVisualGraphics->displayResolutionVector.end(),
              CompareDisplayResolutionAscending());
...
bool VisualGraphics::getNextAvailableDisplayResolution(char *showStr, UInt8 * isSelected)
{
    ...
    if (theVisualGraphics->displayResolutionIterIndex < theVisualGraphics->displayResolutionVector.size()) {
        if ((theVisualGraphics->displayResolutionVector[theVisualGraphics->displayResolutionIterIndex]->
             horizontalPixels ==
             theVisualGraphics->fullscreenDisplayResolution.theFullscreenDisplayResolution.horizontalPixels)
            && (theVisualGraphics->displayResolutionVector[theVisualGraphics->displayResolutionIterIndex]->
                verticalPixels ==
                theVisualGraphics->fullscreenDisplayResolution.theFullscreenDisplayResolution.verticalPixels)) {
            *isSelected = 1;
        } else {
            *isSelected = 0;
        }
        strcpy(showStr,
               theVisualGraphics->displayResolutionVector[theVisualGraphics->displayResolutionIterIndex]->showStr);
...
OSStatus VisualGraphics::matchDisplayResolutionShowStrWithPrefs(const char *const selectedShowStr,
                                                                UInt16 & horizontalPixels, UInt16 & verticalPixels,
                                                                UInt16 & bitsPerPixel, UInt16 & refreshRate)
{
...
void VisualGraphics::evaluateFullscreenDisplayResolution()
{
    ...
    theVisualGraphics->fullscreenDisplayResolution.theFullscreenDisplayResolution.horizontalPixels =
        VisualDataStore::getPreferenceValueInt(VisualConfiguration::kFullscreenWidth, &isDefaultValue);
    if (isDefaultValue == true) {
        // no preferred resolution explicitly set, so we try to evaluate best available resolution
        if (theVisualGraphics->displayResolutionVector.size() > 0) {
            theVisualGraphics->fullscreenDisplayResolution.theFullscreenDisplayResolution =
                *(theVisualGraphics->displayResolutionVector[theVisualGraphics->displayResolutionVector.size() - 1]);
        } else {
            ...
            strcpy(theVisualGraphics->fullscreenDisplayResolution.theFullscreenDisplayResolution.showStr,
                   "unknown best display resolution");
        }
    } else {
        theVisualGraphics->fullscreenDisplayResolution.theFullscreenDisplayResolution.verticalPixels =
            VisualDataStore::getPreferenceValueInt(VisualConfiguration::kFullscreenHeight);
        theVisualGraphics->fullscreenDisplayResolution.theFullscreenDisplayResolution.bitsPerPixel =
            VisualDataStore::getPreferenceValueInt(VisualConfiguration::kFullscreenBitsPerPixel);
        theVisualGraphics->fullscreenDisplayResolution.theFullscreenDisplayResolution.refreshRate =
            VisualDataStore::getPreferenceValueInt(VisualConfiguration::kFullscreenRefreshRate);
    }
...
void VisualGraphics::drawTexture(UInt32 textureNumber, ConstVertexChainRef const vertexChain, bool canUseRectExtension,
...
void VisualGraphics::drawPixels(UInt32 ** pixelData, double xCoord, double yCoord, UInt32 width, UInt32 height,
                                ...
                                const VisualConvolutionFilter * const aConvolutionFilter)
{
    ...
    if (useGLConvolutionFilter == false && aConvolutionFilter != NULL) {
        ...
        aConvolutionFilter->applyToPixelData((const unsigned char *const) *pixelData, width, height, format, dataType,
                                             (unsigned char **) &filteredPixelData);
...
OSStatus VisualGraphics::makeTextureOfStringWin(wchar_t * stringValue,
                                                ...
                                                UInt16 maxPixelWidth, UInt16 maxPixelHeight)
{
//#define useGetDIBits
    ...
    if (resourceFontIsPresent == 0) {
        ...
        if (fontIsInstalled == 0) {
            ...
            OSStatus status =
                VisualFile::getDataOfResource(IDR_DEVROYEFONT, "FONTRESOURCE", &fontData, sizeOfFontResource);
            if (status == noErr) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, lWidthPixels, lHeightPixels, 0, GL_BGRA_EXT, GL_UNSIGNED_BYTE,
                 (GLubyte *) bitmapBits);
...
void VisualGraphics::drawSpot(const float xNum, const float yNum, const float r, const float g, const float b,
                              UInt16 waveformIntensityVal, float intensity, const UInt8 tailSize)
{
                                  ...
                                  const UInt32 numberOfWaveformEntries, SInt16 ** waveformDataMonoArray)
{
    ...
    widthOfWaveform =
        theVisualGraphics->getCanvasCoordWidth() - theVisualGraphics->xPixelToCoord(300) -
        theVisualGraphics->xPixelToCoord(30);
    ...
    glLoadIdentity();
    glTranslated(0.0,
                 theVisualGraphics->maxBottomCoordOfGLView + theVisualGraphics->xPixelToCoord(60) + heightOfWaveform,
                 0.0f);
    ...
    glLoadIdentity();
    glTranslated(0.0,
                 theVisualGraphics->maxBottomCoordOfGLView + theVisualGraphics->xPixelToCoord(10) +
                 (heightOfWaveform / 2.0), 0.0);
                                     ...
                                     const UInt8 *** const spectrumDataArray)
{
    ...
    leftCoord =
        theVisualGraphics->getMaxRightCoordOfCanvas() - widthOfSpectrogram - theVisualGraphics->yPixelToCoord(10);
...
void VisualGraphics::drawWaveformShape(const SInt8 * const waveformShapeArray,
                                       const UInt16 maxNumberOfWaveformShapeHistory)
{
    UInt16 index;
    GLdouble xPoint;
...
void VisualGraphics::createCheckTexture(UInt32 & textureNumber, UInt32 & textureWidth, UInt32 & textureHeight,
...
#if TARGET_OS_MAC
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, textureWidth, textureHeight, 0, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV,
                 (GLuint *) rgbaPixels);
#endif
#if TARGET_OS_WIN
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, textureWidth, textureHeight, 0, GL_BGRA_EXT, GL_UNSIGNED_BYTE,
                 (GLubyte *) rgbaPixels);
#endif
...
OSStatus VisualGraphics::copyARGBBitmapDataToTexture(UInt32 textureNumber, UInt32 width, UInt32 height,
                                                     bool canUseRectExtension, UInt16 format, UInt16 dataType,
                                                     const UInt32 ** bitmapData)
{
...
{
    return ((double) pixelPos *
            (theVisualGraphics->getCanvasCoordWidth() / (double) theVisualGraphics->getCanvasPixelWidth()));
}
...
{
    return ((double) pixelPos *
            (theVisualGraphics->getCanvasCoordHeight() / (double) theVisualGraphics->getCanvasPixelHeight()));
}
...
UInt16 VisualGraphics::xCoordToPixel(double coordPos)
{
    ...
    return (UInt16) (tempCoord *
                     ((double) theVisualGraphics->getCanvasPixelWidth() / theVisualGraphics->getCanvasCoordWidth()));
}
...
UInt16 VisualGraphics::yCoordToPixel(double coordPos)
{
    ...
    return (UInt16) (tempCoord *
                     ((double) theVisualGraphics->getCanvasPixelHeight() / theVisualGraphics->getCanvasCoordHeight()));
}
...
*/
    ...
    // get caps
#if TARGET_OS_MAC
    glGetIntegerv(GL_MAX_TEXTURE_UNITS, &theVisualGraphics->capabilities.textureUnits);
    glGetIntegerv(GL_MAX_3D_TEXTURE_SIZE, &theVisualGraphics->capabilities.max3DTextureSize);
    glGetIntegerv(GL_MAX_CUBE_MAP_TEXTURE_SIZE, &theVisualGraphics->capabilities.maxCubeMapTextureSize);

    // get functionality info
    theVisualGraphics->capabilities.fSpecularVector = gluCheckExtension((GLubyte *) "GL_APPLE_specular_vector", strExt);
    theVisualGraphics->capabilities.fTransformHint = gluCheckExtension((GLubyte *) "GL_APPLE_transform_hint", strExt);
    ...
    theVisualGraphics->capabilities.fClientStorage = gluCheckExtension((GLubyte *) "GL_APPLE_client_storage", strExt);
    theVisualGraphics->capabilities.fYCbCr = gluCheckExtension((GLubyte *) "GL_APPLE_ycbcr_422", strExt);
    theVisualGraphics->capabilities.fTextureRange = gluCheckExtension((GLubyte *) "GL_APPLE_texture_range", strExt);
    theVisualGraphics->capabilities.fFence = gluCheckExtension((GLubyte *) "GL_APPLE_fence", strExt);
    theVisualGraphics->capabilities.fVAR = gluCheckExtension((GLubyte *) "GL_APPLE_vertex_array_range", strExt);
    theVisualGraphics->capabilities.fVAO = gluCheckExtension((GLubyte *) "GL_APPLE_vertex_array_object", strExt);
    theVisualGraphics->capabilities.fElementArray = gluCheckExtension((GLubyte *) "GL_APPLE_element_array", strExt);
    theVisualGraphics->capabilities.fVPEvals =
        gluCheckExtension((GLubyte *) "GL_APPLE_vertex_program_evaluators", strExt);
    theVisualGraphics->capabilities.fFloatPixels = gluCheckExtension((GLubyte *) "GL_APPLE_float_pixels", strExt);
    theVisualGraphics->capabilities.fFlushRenderer = gluCheckExtension((GLubyte *) "GL_APPLE_flush_render", strExt);
    theVisualGraphics->capabilities.fPixelBuffer = gluCheckExtension((GLubyte *) "GL_APPLE_pixel_buffer", strExt);
    theVisualGraphics->capabilities.fImaging = gluCheckExtension((GLubyte *) "GL_ARB_imaging", strExt);
    ...
    theVisualGraphics->capabilities.fPointParam =
        ...
        (theVisualGraphics->capabilities.glVersion >= 0x0140);
    theVisualGraphics->capabilities.fVertexProg = gluCheckExtension((GLubyte *) "GL_ARB_vertex_program", strExt);
    theVisualGraphics->capabilities.fFragmentProg = gluCheckExtension((GLubyte *) "GL_ARB_fragment_program", strExt);
    ...
    theVisualGraphics->capabilities.fShadow =
        gluCheckExtension((GLubyte *) "GL_ARB_shadow", strExt) || (theVisualGraphics->capabilities.glVersion >= 0x0140);
    theVisualGraphics->capabilities.fShadowAmbient = gluCheckExtension((GLubyte *) "GL_ARB_shadow_ambient", strExt);
    theVisualGraphics->capabilities.fVertexBlend = gluCheckExtension((GLubyte *) "GL_ARB_vertex_blend", strExt);
    ...
    theVisualGraphics->capabilities.fTex3D =
        ...
        (theVisualGraphics->capabilities.glVersion >= 0x0120);
    theVisualGraphics->capabilities.fClipVolHint = gluCheckExtension((GLubyte *) "GL_EXT_clip_volume_hint", strExt);
    ...
    theVisualGraphics->capabilities.fBlendSub =
        ...
        gluCheckExtension((GLubyte *) "GL_ARB_imaging", strExt);
    theVisualGraphics->capabilities.fCVA = gluCheckExtension((GLubyte *) "GL_EXT_compiled_vertex_array", strExt);
    theVisualGraphics->capabilities.fTexLODBias =
        gluCheckExtension((GLubyte *) "GL_EXT_texture_lod_bias", strExt) ||
        (theVisualGraphics->capabilities.glVersion >= 0x0140);
    theVisualGraphics->capabilities.fABGR = gluCheckExtension((GLubyte *) "GL_EXT_abgr", strExt);
    theVisualGraphics->capabilities.fBGRA =
        gluCheckExtension((GLubyte *) "GL_EXT_bgra", strExt) || (theVisualGraphics->capabilities.glVersion >= 0x0120);
    theVisualGraphics->capabilities.fTexFilterAniso =
        gluCheckExtension((GLubyte *) "GL_EXT_texture_filter_anisotropic", strExt);
    theVisualGraphics->capabilities.fPaletteTex = gluCheckExtension((GLubyte *) "GL_EXT_paletted_texture", strExt);
    ...
    theVisualGraphics->capabilities.fTexRect = gluCheckExtension((GLubyte *) "GL_EXT_texture_rectangle", strExt);
    theVisualGraphics->capabilities.fFogCoord = gluCheckExtension((GLubyte *) "GL_EXT_fog_coord", strExt);
    ...
    theVisualGraphics->capabilities.fMultiDrawArrays =
        ...
        (theVisualGraphics->capabilities.glVersion >= 0x0140);
    theVisualGraphics->capabilities.fShadowFunc = gluCheckExtension((GLubyte *) "GL_EXT_shadow_funcs", strExt);
    ...
    theVisualGraphics->capabilities.fTexEdgeClamp =
        ...
        (theVisualGraphics->capabilities.glVersion >= 0x0120);
    theVisualGraphics->capabilities.fGenMipmap = gluCheckExtension((GLubyte *) "GL_SGIS_generate_mipmap", strExt);
    theVisualGraphics->capabilities.fTexLOD =
        gluCheckExtension((GLubyte *) "GL_SGIS_texture_lod", strExt) ||
        (theVisualGraphics->capabilities.glVersion >= 0x0120);
    theVisualGraphics->capabilities.fPointCull = gluCheckExtension((GLubyte *) "GL_ATI_point_cull_mode", strExt);
    ...
    theVisualGraphics->capabilities.fCombine3 = gluCheckExtension((GLubyte *) "GL_ATI_texture_env_combine3", strExt);
    theVisualGraphics->capabilities.fSepStencil = gluCheckExtension((GLubyte *) "GL_ATI_separate_stencil", strExt);
    theVisualGraphics->capabilities.fArrayRevComps4Byte =
        gluCheckExtension((GLubyte *) "GL_ATI_array_rev_comps_in_4_bytes", strExt);
    theVisualGraphics->capabilities.fPointSprite = gluCheckExtension((GLubyte *) "GL_NV_point_sprite", strExt);
    theVisualGraphics->capabilities.fRegCombiners = gluCheckExtension((GLubyte *) "GL_NV_register_combiners", strExt);
    theVisualGraphics->capabilities.fRegCombiners2 = gluCheckExtension((GLubyte *) "GL_NV_register_combiners2", strExt);
    ...
    theVisualGraphics->capabilities.fBlendSquare =
        ...
        (theVisualGraphics->capabilities.glVersion >= 0x0140);
    theVisualGraphics->capabilities.fFogDist = gluCheckExtension((GLubyte *) "GL_NV_fog_distance", strExt);
    theVisualGraphics->capabilities.fMultisampleFilterHint =
        gluCheckExtension((GLubyte *) "GL_NV_multisample_filter_hint", strExt);
    theVisualGraphics->capabilities.fTexGenReflect = gluCheckExtension((GLubyte *) "GL_NV_texgen_reflection", strExt);
    theVisualGraphics->capabilities.fTexShader = gluCheckExtension((GLubyte *) "GL_NV_texture_shader", strExt);
    theVisualGraphics->capabilities.fTexShader2 = gluCheckExtension((GLubyte *) "GL_NV_texture_shader2", strExt);
    theVisualGraphics->capabilities.fTexShader3 = gluCheckExtension((GLubyte *) "GL_NV_texture_shader3", strExt);
    theVisualGraphics->capabilities.fDepthClamp = gluCheckExtension((GLubyte *) "GL_NV_depth_clamp", strExt);
    theVisualGraphics->capabilities.fLightMaxExp = gluCheckExtension((GLubyte *) "GL_NV_light_max_exponent", strExt);
    theVisualGraphics->capabilities.fRasterPosClip = gluCheckExtension((GLubyte *) "GL_IBM_rasterpos_clip", strExt);
...
void VisualGraphics::copyFramebufferToTexture(UInt32 textureNumber, bool canUseRectExtension, UInt32 xPos, UInt32 yPos,
                                              UInt32 width, UInt32 height, UInt16 pixelFormat, UInt16 dataType)
{
...
#if TARGET_OS_MAC
            glCopyTexImage2D(GL_TEXTURE_RECTANGLE_EXT, 0, GL_RGBA, drawRectOriginXPos, drawRectOriginYPos, width,
                             height, 0);
#endif
        }
    } else {
...
#if TARGET_OS_MAC
            glTexImage2D(GL_TEXTURE_RECTANGLE_EXT, 0, GL_RGBA, width, height, 0, pixelFormat, dataType,
                         (GLuint *) image);
#endif
        }
        free(image);
