...
void VisualActorGraphics::createCheckTexture(UInt32 & textureNumber, UInt32 & textureWidth, UInt32 & textureHeight,
...
bool VisualActorGraphics::getCoordsOfPixelPosition(double xPixelPos,
                                                   double yPixelPos,
                                                   double zAxisPos,
                                                   double *xCoordPos, double *yCoordPos, double *zCoordPos)
{
    VisualGraphics *theVisualGraphics = VisualGraphics::getInstance();
    return theVisualGraphics->getCoordsOfPixelPosition(xPixelPos + theVisualGraphics->getCanvasXOriginOffset(),
                                                       yPixelPos + theVisualGraphics->getCanvasYOriginOffset(),
                                                       zAxisPos, xCoordPos, yCoordPos, zCoordPos);
}
...
bool VisualActorGraphics::getPixelsOfCoordPosition(double xCoordPos,
                                                   double yCoordPos,
                                                   double zCoordPos,
                                                   double *xPixelPos, double *yPixelPos, double *zAxisPos)
{
    VisualGraphics *theVisualGraphics = VisualGraphics::getInstance();
...
void VisualActorGraphics::drawSpot(const float xNum, const float yNum, const float r, const float g, const float b,
...
void VisualActorGraphics::drawWaveform(const SInt16 historyNum, const UInt16 maxNumberOfHistories,
                                       const UInt32 numberOfWaveformEntries,
                                       const SInt16 ** const waveformDataMonoArray)
{
    VisualGraphics *theVisualGraphics = VisualGraphics::getInstance();
    theVisualGraphics->drawWaveform(historyNum, maxNumberOfHistories, numberOfWaveformEntries,
                                    const_cast < SInt16 ** >(waveformDataMonoArray));
}


void VisualActorGraphics::drawSpectrumAnalyzer(const SInt16 currHistoryNum, const UInt16 numberOfHistories,
                                               const UInt32 numberOfSpectrumEntries, const UInt16 numberOfAudioChannels,
                                               const UInt8 *** const spectrumDataArray)
{
    VisualGraphics *theVisualGraphics = VisualGraphics::getInstance();
    theVisualGraphics->drawSpectrumAnalyzer(currHistoryNum, numberOfHistories, numberOfSpectrumEntries,
                                            numberOfAudioChannels, spectrumDataArray);
}


void VisualActorGraphics::drawSpectrogram(const SInt16 currHistoryNum, const UInt16 numberOfHistories,
                                          const UInt32 numberOfSpectrumEntries, const UInt16 numberOfAudioChannels,
                                          const UInt8 *** const spectrumDataArray)
{
    VisualGraphics *theVisualGraphics = VisualGraphics::getInstance();
    theVisualGraphics->drawSpectrogram(currHistoryNum, numberOfHistories, numberOfSpectrumEntries,
                                       numberOfAudioChannels, spectrumDataArray);
}
...
void VisualActorGraphics::prepareCoverArtAction()
{
    ...
    theVisualGraphics->translateMatrix(0.0, 0.0,
                                       (theVisualGraphics->getMaxFarCoordOfCanvas() -
                                        theVisualGraphics->getMaxNearCoordOfCanvas()) / 2.0 * -1.0 -
                                       theVisualGraphics->getMaxNearCoordOfCanvas());
}
