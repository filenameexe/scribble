...
typedef OSStatus(*GetDimensionsOfStringBitmapProc) (CFStringRef stringValue, UInt32 * bitmapWidth,
                                                    UInt32 * bitmapHeight, const char *const fontNameStr,
...
typedef OSStatus(*GetStringBitmapDataProc) (CFStringRef stringValue, UInt32 bitmapWidth, UInt32 bitmapHeight,
                                            const char *const fontNameStr, float fontSize, float red, float green,
                                            float blue, const char *const alignment, UInt32 ** bitmapOut);
...
void monitorRenderMessageProcess()
{
    ...
    // screen measurements
    sprintf(cStr64, "width: %d, height: %d, refreshRate: %d", theVisualGraphics->getScreenWidth(),
            theVisualGraphics->getScreenHeight(), theVisualGraphics->getRefreshRateOfScreen());
    ...
    sprintf(cStr64, "top: %.2f, left: %.2f", theVisualGraphics->getMaxTopCoordOfCanvas(),
            theVisualGraphics->getMaxLeftCoordOfCanvas());
    setProcessInfo("CoordMaxTopLeft", cStr64);
    sprintf(cStr64, "bottom: %.2f, right: %.2f", theVisualGraphics->getMaxBottomCoordOfCanvas(),
            theVisualGraphics->getMaxRightCoordOfCanvas());
    setProcessInfo("CoordMaxBottomRight", cStr64);
    sprintf(cStr64, "near: %.2f, far: %.2f", theVisualGraphics->getMaxNearCoordOfCanvas(),
            theVisualGraphics->getMaxFarCoordOfCanvas());
    setProcessInfo("CoordMaxNearFar", cStr64);

    theVisualGraphics->getViewportRect(&viewportRect);
    sprintf(cStr64, "top: %d, left: %d, bottom: %d, right: %d", viewportRect.top, viewportRect.left,
            viewportRect.bottom, viewportRect.right);
...
void setRefsToCocoaFunctionCalls()
{
    ...
    if (!bundleRef) {
        ...
        doInitCocoaForCarbonBundle =
            (InitCocoaForCarbonBundleProc) CFBundleGetFunctionPointerForName(bundleRef,
                                                                             CFSTR("initCocoaForCarbonBundle"));
        if (!doInitCocoaForCarbonBundle) {
            writeLog("no function pointer for doInitCocoaForCarbonBundle");
        }
        doGetDimensionsOfStringBitmap =
            (GetDimensionsOfStringBitmapProc) CFBundleGetFunctionPointerForName(bundleRef,
                                                                                CFSTR("getDimensionsOfStringBitmap"));
        if (!doGetDimensionsOfStringBitmap) {
            writeLog("no function pointer for doGetDimensionsOfStringBitmap");
        }
        doGetStringBitmapData =
            (GetStringBitmapDataProc) CFBundleGetFunctionPointerForName(bundleRef, CFSTR("getStringBitmapData"));
...
OSStatus getDimensionsOfCocoaStringBitmap(void *aVisualString, UInt32 * imageWidth, UInt32 * imageHeight,
                                          char *fontNameStr, float *fontSize, float red, float green, float blue,
                                          UInt16 maxPixelWidth, UInt16 maxPixelHeight, char *alignment)
{
    ...
    err =
        doGetDimensionsOfStringBitmap(aCFStringRef, imageWidth, imageHeight, fontNameStr, fontSize, red, green, blue,
                                      maxPixelWidth, maxPixelHeight, alignment);
...
OSStatus getCocoaStringBitmapData(void *aVisualString, UInt32 bitmapWidth, UInt32 bitmapHeight, char *fontNameStr,
                                  float fontSize, float red, float green, float blue, char *alignment,
                                  UInt32 ** bitmapData)
{
    ...
    err =
        doGetStringBitmapData(aCFStringRef, bitmapWidth, bitmapHeight, fontNameStr, fontSize, red, green, blue,
                              alignment, bitmapData);
...
OSStatus copyBitmapDataToTexture(UInt32 textureNumber, UInt32 width, UInt32 height, bool canUseRectExtension,
                                 UInt16 format, UInt16 dataType, UInt32 ** bitmapData)
{
    VisualGraphics *theVisualGraphics = VisualGraphics::getInstance();
    return theVisualGraphics->copyARGBBitmapDataToTexture(textureNumber, width, height, canUseRectExtension, format,
                                                          dataType, const_cast < const UInt32 ** >(bitmapData));
}
