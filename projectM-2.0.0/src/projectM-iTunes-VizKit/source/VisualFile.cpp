...
OSStatus VisualFile::getData(void **data, UInt32 & size)
{
    ...
    while (readOn == true) {
        ...
        if (status != noErr) {
            ...
            if (debug == true) {
                sprintf(logStr, "OK: readData (cnt: %d, totalBytes: %ld, req: %ld, read: %ld)", bufferFillCount,
                        numberOfBytesRead, requestCount, actualCount);
                writeLog(logStr);
...
bool VisualFile::doesExist(void)
{
    ...
    if (this->knownWhetherFileOrDirectoryExists == false) {
        this->doesExistBool = this->knownWhetherFileOrDirectoryExists = true;
    }
#endif
