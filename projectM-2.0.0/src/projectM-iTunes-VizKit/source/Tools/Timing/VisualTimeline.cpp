...
void VisualTimeline::setCurrentValue(double newValue)
{
    ...
    if (distance == 0.0)
        return;                 // prevent divison by zero
    if (this->movingDirection == kForward) {
        if (currStartValue < currEndValue) {
            this->offsetMilliseconds =
                (UInt32) ((double) this->durationInMilliseconds * ((newValue - currStartValue) / distance));
        } else {
            this->offsetMilliseconds =
                (UInt32) ((double) this->durationInMilliseconds * ((currStartValue - newValue) / distance));
        }
    } else if (this->movingDirection == kBackward) {
        if (currStartValue < currEndValue) {
            this->offsetMilliseconds =
                (UInt32) ((double) this->durationInMilliseconds * ((currEndValue - newValue) / distance));
        } else {
            this->offsetMilliseconds =
                (UInt32) ((double) this->durationInMilliseconds * ((newValue - currEndValue) / distance));
        }
    }
}
