...
OSStatus VisualQuickTime::getGraphicsImporterForFile(const VisualFile & aFile,
                                                     GraphicsImportComponent & importComponent, OSType fileType)
{
    OSErr err = noErr;
...
OSStatus VisualQuickTime::getGraphicsImporterForHandle(Handle dataHandle, GraphicsImportComponent & importComponent,
                                                       OSType fileType)
{
    OSErr err = noErr;
...
OSStatus VisualQuickTime::flipImportMatrix(GraphicsImportComponent & importComponent, UInt32 imageWidth,
                                           UInt32 imageHeight)
{
    MatrixRecord matrix;
...
CGImageRef VisualQuickTime::createCGImageRef(GraphicsImportComponent & importComponent)
{
    ...
    ComponentResult compErr =
        GraphicsImportCreateCGImage(importComponent, &imageRef, kGraphicsImportCreateCGImageUsingCurrentSettings);
    if (compErr != noErr) {
        char errLog[256];
        sprintf(errLog, "GraphicsImportCreateCGImage err (%ld) in file: %s (line: %d) [%s])", compErr, __FILE__,
...
UInt32 *VisualQuickTime::getBitmapOfDrawing(GraphicsImportComponent & importComponent, UInt32 width, UInt32 height)
{
    ...
    if (compErr != noErr) {
        sprintf(errLog, "GraphicsImportSetGWorld err (%ld) in file: %s (line: %d) [%s])", compErr, __FILE__, __LINE__,
                __FUNCTION__);
        writeLog(errLog);
    }
    ...
    if (compErr == noErr) {
        if (GetGWorldPixMap(coverArtGWorld) && LockPixels(GetGWorldPixMap(coverArtGWorld))) {
            ...
            if (compErr != noErr) {
                sprintf(errLog, "GraphicsImportSetGraphicsMode err (%ld) in file: %s (line: %d) [%s])", compErr,
    ...
    if (compErr != noErr) {
        sprintf(errLog, "GraphicsImportDraw err (%ld) in file: %s (line: %d) [%s])", compErr, __FILE__, __LINE__,
                __FUNCTION__);
        writeLog(errLog);
...
OSStatus VisualQuickTime::writeARGBBitmapToFile(UInt32 * argbBitmap, const VisualFile & aFile, UInt32 width,
                                                UInt32 height, OSType aFileFormatType)
{
...
#if TARGET_OS_MAC
    osErr =
        QTNewGWorldFromPtr(&imageGWorld, pixelFormat, &destRect, NULL, NULL, kNativeEndianPixMap, argbBitmap,
                           width * 4);
#endif
#if TARGET_OS_WIN
...
OSStatus VisualQuickTime::getGraphicsImporterComponentForFileType(OSType fileType,
