...
std::auto_ptr < Preset > PresetLoader::loadPreset(unsigned int index) const const
{
    ...
    return _presetFactoryManager.factory(extension).allocate(_entries[index], _presetNames[index]);

}


std::auto_ptr < Preset > PresetLoader::loadPreset(const std::string & url) const const
{
    ...
    return _presetFactoryManager.factory(extension).allocate(url, url);
...
void PresetLoader::handleDirectoryError()
{
    ...
    switch (errno) {
    case ENOENT:
        std::cerr << "[PresetLoader] ENOENT error. The path \"" << this->
            _dirname << "\" probably does not exist. \"man open\" for more info." << std::endl;
        break;
    case ENOMEM:
...
const std::string & PresetLoader::getPresetURL(unsigned int index) const const
{
    return _entries[index];
}

const std::string & PresetLoader::getPresetName(unsigned int index) const const
{
    return _presetNames[index];
}

int PresetLoader::getPresetRating(unsigned int index, const PresetRatingType ratingType) const const
{
    return _ratings[ratingType][index];
}

const std::vector < RatingList > &PresetLoader::getPresetRatings() const const
{
    return _ratings;
}

const std::vector < int >&PresetLoader::getPresetRatingsSums() const const
{
    return _ratingsSums;
}
...
void PresetLoader::insertPresetURL(unsigned int index, const std::string & url, const std::string & presetName,
