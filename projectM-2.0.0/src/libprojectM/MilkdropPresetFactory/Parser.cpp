...
int Parser::parse_line(std::istream & fs, MilkdropPreset * preset)
{
    ...
    case tDiv:
        ...
        if (PARSE_DEBUG)
            std::cerr << "parse_line: invalid token found at start of line (LINE " << line_count << ")" << std::endl;
...
//      char z = fs.get();
        ...
        /* Custom Wave Prefix */
        if ((!strncmp(eqn_string, WAVE_STRING, WAVE_STRING_LENGTH)) && ((eqn_string[5] >= 48) && (eqn_string[5] <= 57))) {
            tokenWrapAroundEnabled = true;
        ...
        if (!strncmp(eqn_string, SHAPECODE_STRING, SHAPECODE_STRING_LENGTH)) {
            ...
            if (PARSE_DEBUG)
                printf("parse_line: shapecode prefix found: \"%s\"\n", eqn_string);
        ...
        if ((!strncmp(eqn_string, SHAPE_STRING, SHAPE_STRING_LENGTH)) &&
            ...
            tokenWrapAroundEnabled = true;
            if (PARSE_DEBUG)
                printf("parse_line shape prefix found: \"%s\"\n", eqn_string);
        ...
        if (!strncmp(eqn_string, PER_PIXEL_STRING, PER_PIXEL_STRING_LENGTH)) {
            ...
            if (PARSE_DEBUG)
                printf("parse_line: finished parsing per pixel equation (LINE %d)\n", line_count);
            return PROJECTM_SUCCESS;
        }
        ...
        if (line_mode == PER_FRAME_LINE_MODE) {
            ...
            tokenWrapAroundEnabled = true;
            if (PARSE_DEBUG)
                printf("parse_line: parsing implicit per frame init eqn)\n");
            if ((init_cond = parse_per_frame_init_eqn(fs, preset, NULL)) == NULL) {
                tokenWrapAroundEnabled = false;
                return PROJECTM_PARSE_ERROR;
            }
            ...
            tokenWrapAroundEnabled = true;
            if (PARSE_DEBUG)
                printf("parse_line: implicit per pixel eqn (LINE %d)\n", line_count);
            ...
            tokenWrapAroundEnabled = true;
            if (PARSE_DEBUG)
                printf("parse_line: implicit cwave ppoint eqn found (LINE %d)\n", line_count);
            //int len = strlen(eqn_string);

            if (parse_wave_helper(fs, preset, last_custom_wave_id, last_eqn_type, eqn_string) < 0) {
                if (PARSE_DEBUG)
                    printf("parse_line: failed to parse an implicit custom wave per point eqn\n");
                return PROJECTM_FAILURE;
            }
            return PROJECTM_SUCCESS;
            ...
            if (PARSE_DEBUG)
                printf("unsupported line mode: CUSTOM_WAVE_WAVECODE_LINE_MODE\n");
            return PROJECTM_FAILURE;
        } else if (line_mode == CUSTOM_SHAPE_SHAPECODE_LINE_MODE) {
            if (PARSE_DEBUG)
                printf("unsupported line mode: CUSTOM_SHAPE_SHAPECODE_LINE_MODE\n");
            return PROJECTM_FAILURE;
        ...
        if (PARSE_DEBUG)
            printf("parse_line: found initial condition: name = \"%s\" (LINE %d)\n", eqn_string, line_count);
        /* Evaluate the initial condition */
        if ((init_cond = parse_init_cond(fs, eqn_string, preset)) == NULL) {
            if (PARSE_DEBUG)
                printf("parse_line: failed to parse initial condition (LINE %d)\n", line_count);
            return PROJECTM_PARSE_ERROR;
        }
    ...
    default:                   /* an uncaught type or an error has occurred */
        if (PARSE_DEBUG)
            printf("parse_line: uncaught case, token val = %d\n", token);
        return PROJECTM_PARSE_ERROR;
    }
...
GenExpr *Parser::parse_gen_expr(std::istream & fs, TreeExpr * tree_expr, MilkdropPreset * preset)
{
    ...
    case tLPr:
        ...
        if ((func = BuiltinFuncs::find_func(string)) != NULL) {
            ...
            if ((gen_expr =
                 GenExpr::prefun_to_expr((float (*)(void *)) func->func_ptr, expr_list, func->getNumArgs())) == NULL) {
                if (PARSE_DEBUG)
            ...
            if (*string != 0) {
                if (PARSE_DEBUG)
                    printf("parse_prefix_args: empty string expected, but not found...(LINE %d)\n", line_count);
        ...
        if (*string != 0) {
            ...
            if (PARSE_DEBUG)
                printf("parse_gen_expr: implicit multiplication case unimplemented!\n");
            if (tree_expr)
                delete tree_expr;
            return NULL;
        }
        ...
        if ((gen_expr = parse_gen_expr(fs, NULL, preset)) == NULL) {
            if (PARSE_DEBUG)
                printf("parse_gen_expr:  found left parentice, but failed to create new expression tree \n");
            if (tree_expr)
                delete tree_expr;
            return NULL;
        }
        ...
        if (PARSE_DEBUG)
            printf("parse_gen_expr: finished enclosed expression tree...\n");
        token = parseToken(fs, string);
        return parse_infix_op(fs, token, insert_gen_expr(gen_expr, &tree_expr), preset);
    ...
    case tPlus:
        if (*string == 0) {
            ...
            if (PARSE_DEBUG)
                printf("parse_gen_expr: plus used as prefix (LINE %d)\n", line_count);
    ...
    case tComma:
        ...
        if (*string == 0) {
            if (PARSE_DEBUG)
                printf("parse_gen_expr: empty string coupled with terminal (LINE %d) \n", line_count);
    ...
    default:
        ...
        if (current_shape != NULL) {
            if ((param =
                 ParamUtils::find < ParamUtils::NO_CREATE > (std::string(string),
                                                             &current_shape->param_tree)) == NULL) {
                if ((param = preset->builtinParams.find_builtin_param(std::string(string))) == NULL)
                    if ((param =
                         ParamUtils::find < ParamUtils::AUTO_CREATE > (std::string(string),
                                                                       &current_shape->param_tree)) == NULL) {
                        if (tree_expr)
                            delete tree_expr;
                        return NULL;
            ...
            if (PARSE_DEBUG) {
                std::cerr << "parse_gen_expr: custom shape parameter (name = " << param->name << ")" << std::endl;
            }
        ...
        if (current_wave != NULL) {
            if ((param =
                 ParamUtils::find < ParamUtils::NO_CREATE > (std::string(string), &current_wave->param_tree)) == NULL) {
                if ((param = preset->builtinParams.find_builtin_param(std::string(string))) == NULL)
                    if ((param =
                         ParamUtils::find < ParamUtils::AUTO_CREATE > (std::string(string),
                                                                       &current_wave->param_tree)) == NULL) {
                        if (tree_expr)
                            delete tree_expr;
                        return NULL;
                    }
...
int Parser::insert_gen_rec(GenExpr * gen_expr, TreeExpr * root)
{
    ...
    if (PARSE_DEBUG)
        printf("insert_gen_rec: should never reach here!\n");
    return PROJECTM_FAILURE;
}
...
GenExpr *Parser::parse_infix_op(std::istream & fs, token_t token, TreeExpr * tree_expr, MilkdropPreset * preset)
{
    ...
    case tPlus:
        if (PARSE_DEBUG)
            printf("parse_infix_op: found addition operator (LINE %d)\n", line_count);
        if (PARSE_DEBUG)
    ...
    case tComma:
        if (PARSE_DEBUG)
            printf("parse_infix_op: terminal found (LINE %d)\n", line_count);
        gen_expr = new GenExpr(TREE_T, (void *) tree_expr);
        assert(gen_expr);
        return gen_expr;
    default:
        if (PARSE_DEBUG)
            printf("parse_infix_op: operator or terminal expected, but not found (LINE %d)\n", line_count);
        delete tree_expr;
        return NULL;
    }
...
int Parser::parse_float(std::istream & fs, float *float_ptr)
{
    ...
    if (PARSE_DEBUG)
        printf("parse_float: float conversion failed for string \"%s\"\n", string);
...
PerFrameEqn *Parser::parse_per_frame_eqn(std::istream & fs, int index, MilkdropPreset * preset)
{
    ...
    if (PARSE_DEBUG)
        std::cerr << "parse_per_frame_eqn: parameter \"" << param->
            name << "\" retrieved (LINE" << line_count << ")" << std::endl;
    /* Make sure parameter is writable */
    if (param->flags & P_FLAG_READONLY) {
        if (PARSE_DEBUG)
            std::cerr << "parse_per_frame_eqn: parameter \"" << param->
                name << "\" %s is marked as read only (LINE " << line_count << ")" << std::endl;
        return NULL;
    }
...
PerFrameEqn *Parser::parse_implicit_per_frame_eqn(std::istream & fs, char *param_string, int index,
                                                  MilkdropPreset * preset)
{
    ...
    if (param->flags & P_FLAG_READONLY) {
        if (PARSE_DEBUG)
            printf("parse_implicit_per_frame_eqn: parameter %s is marked as read only (LINE %d)\n", param->name.c_str(),
                   line_count);
        return NULL;
    }
    ...
    if ((gen_expr = parse_gen_expr(fs, NULL, preset)) == NULL) {
        if (PARSE_DEBUG)
            printf("parse_implicit_per_frame_eqn: equation evaluated to null (LINE %d)\n", line_count);
        return NULL;
    }

    if (PARSE_DEBUG)
        printf("parse_implicit_per_frame_eqn: finished per frame equation evaluation (LINE %d)\n", line_count);

    /* Create a new per frame equation */
    if ((per_frame_eqn = new PerFrameEqn(index, param, gen_expr)) == NULL) {
        if (PARSE_DEBUG)
            printf("parse_implicit_per_frame_eqn: failed to create a new per frame eqn, out of memory?\n");
        delete gen_expr;
        return NULL;
    }

    if (PARSE_DEBUG)
        printf("parse_implicit_per_frame_eqn: per_frame eqn parsed succesfully\n");
...
InitCond *Parser::parse_init_cond(std::istream & fs, char *name, MilkdropPreset * preset)
{
    ...
    if (PARSE_DEBUG)
        printf("parse_init_cond: parameter = \"%s\" (LINE %d)\n", param->name.c_str(), line_count);

    if (param->flags & P_FLAG_READONLY) {
        if (PARSE_DEBUG)
            printf("parse_init_cond: builtin parameter \"%s\" marked as read only!\n", param->name.c_str());
        return NULL;
    }
    ...
    if (PARSE_DEBUG)
        printf("parsed_init_cond: parsing initial condition value... (LINE %d)\n", line_count);
    ...
    if ((param->type == P_TYPE_BOOL)) {
        ...
        if ((parse_int(fs, &bool_test)) == PROJECTM_PARSE_ERROR) {
            if (PARSE_DEBUG)
                printf("parse_init_cond: error parsing integer!\n");
            return NULL;
        }
    ...
    /* float value */
        ...
        if ((parse_float(fs, (float *) &init_val.float_val)) == PROJECTM_PARSE_ERROR) {
            if (PARSE_DEBUG)
                printf("parse_init_cond: error parsing float!\n");
            return NULL;
        }
    }
    ...
    /* Unknown value */
        ...
        if (PARSE_DEBUG)
            printf("parse_init_cond: unknown parameter type!\n");
        return NULL;
    }
    ...
    if ((init_cond = new InitCond(param, init_val)) == NULL) {
        if (PARSE_DEBUG)
            printf("parse_init_cond: new_init_cond failed!\n");
        return NULL;
    }
...
InitCond *Parser::parse_per_frame_init_eqn(std::istream & fs, MilkdropPreset * preset, std::map < std::string,
                                           Param * >*database)
{
    ...
    if (PARSE_DEBUG)
        printf("parse_per_frame_init_eqn: parameter = \"%s\" (LINE %d)\n", param->name.c_str(), line_count);

    if (param->flags & P_FLAG_READONLY) {
        if (PARSE_DEBUG)
            printf("pars_per_frame_init_eqn: builtin parameter \"%s\" marked as read only!\n", param->name.c_str());
        return NULL;
    }
    ...
    if (PARSE_DEBUG)
        printf("parse_per_frame_init_eqn: parsing right hand side of per frame init equation.. (LINE %d)\n",
               line_count);

    if ((gen_expr = parse_gen_expr(fs, NULL, preset)) == NULL) {
        if (PARSE_DEBUG)
            printf("parse_per_frame_init_eqn: failed to parse general expresion!\n");
        return NULL;
    }
    ...
    /* Unknown value */
        ...
        if (PARSE_DEBUG)
            printf("pase_per_frame_init_eqn: unknown parameter type!\n");
        return NULL;
    }
    ...
    if ((init_cond = new InitCond(param, init_val)) == NULL) {
        if (PARSE_DEBUG)
            printf("parse_per_frame_init_eqn: new_init_cond failed!\n");
        return NULL;
    }
...
int Parser::parse_wavecode(char *token, std::istream & fs, MilkdropPreset * preset)
{
    ...
    if (PARSE_DEBUG)
        printf("parse_wavecode: wavecode id = %d, parameter = \"%s\"\n", id, var_string);
    ...
    if ((custom_wave = MilkdropPreset::find_custom_object(id, preset->customWaves)) == NULL) {
        std::cerr << "parse_wavecode: failed to load (or create) custom wave (id = " << id << ")!\n" << std::endl;

        return PROJECTM_FAILURE;
    }

    if (PARSE_DEBUG)
        printf("parse_wavecode: custom wave found (id = %d)\n", custom_wave->id);
    ...
    if (PARSE_DEBUG)
        printf("parse_wavecode: custom wave parameter found (name = %s)\n", param->name.c_str());
    ...
    if ((param->type == P_TYPE_BOOL)) {
        ...
        if ((parse_int(fs, &bool_test)) == PROJECTM_PARSE_ERROR) {
            ...
            if (PARSE_DEBUG)
                printf("parse_wavecode: error parsing integer!\n");
            return PROJECTM_PARSE_ERROR;
        }
        init_val.bool_val = bool_test;
    } else if ((param->type == P_TYPE_INT)) {
    ...
    /* float value */
        ...
        if ((parse_float(fs, (float *) &init_val.float_val)) == PROJECTM_PARSE_ERROR) {
            if (PARSE_DEBUG)
                printf("parse_wavecode: error parsing float!\n");
            return PROJECTM_PARSE_ERROR;
        }
    }
    ...
    /* Unknown value */
        ...
        if (PARSE_DEBUG)
            printf("parse_wavecode: unknown parameter type!\n");
        return PROJECTM_PARSE_ERROR;
    }
    ...
    if (init_cond == NULL) {
        if (PARSE_DEBUG)
            printf("parse_wavecode: new_init_cond failed!\n");
        return PROJECTM_FAILURE;
    }
...
int Parser::parse_shapecode(char *token, std::istream & fs, MilkdropPreset * preset)
{
    ...
    if (PARSE_DEBUG)
        printf("parse_shapecode: shapecode id = %d, parameter = \"%s\"\n", id, var_string);
    ...
    if ((custom_shape = MilkdropPreset::find_custom_object(id, preset->customShapes)) == NULL) {
        if (PARSE_DEBUG)
            printf("parse_shapecode: failed to load (or create) custom shape (id = %d)!\n", id);
        return PROJECTM_FAILURE;
    }
    if (PARSE_DEBUG)
        printf("parse_shapecode: custom shape found (id = %d)\n", custom_shape->id);
    ...
    if ((param = ParamUtils::find < ParamUtils::NO_CREATE > (var_string, &custom_shape->text_properties_tree)) != NULL) {
        ...
        *((std::string *) param->engine_val) = text;
        if (PARSE_DEBUG)
            std::cerr << "parse_shapecode: found image url, text is \"" << text << "\"" << std::endl;
    ...
    if ((param = ParamUtils::find < ParamUtils::AUTO_CREATE > (var_string, &custom_shape->param_tree)) == NULL) {
        if (PARSE_DEBUG)
            printf("parse_shapecode: failed to create parameter.\n");
        return PROJECTM_FAILURE;
    }
    if (PARSE_DEBUG)
        printf("parse_shapecode: custom shape parameter found (name = %s)\n", param->name.c_str());
    ...
    if ((param->type == P_TYPE_BOOL)) {
        ...
        if ((parse_int(fs, &bool_test)) == PROJECTM_PARSE_ERROR) {
            if (PARSE_DEBUG)
                printf("parse_shapecode: error parsing integer!\n");
            return PROJECTM_PARSE_ERROR;
        }
        init_val.bool_val = bool_test;
    } else if ((param->type == P_TYPE_INT)) {
    ...
    /* float value */
        ...
        if ((parse_float(fs, (float *) &init_val.float_val)) == PROJECTM_PARSE_ERROR) {
            if (PARSE_DEBUG)
                printf("parse_shapecode: error parsing float!\n");
            return PROJECTM_PARSE_ERROR;
        }
    }
    ...
    /* Unknown value */
        ...
        if (PARSE_DEBUG)
            printf("parse_shapecode: unknown parameter type!\n");
        return PROJECTM_PARSE_ERROR;
    }
    ...
    if ((init_cond = new InitCond(param, init_val)) == NULL) {
        if (PARSE_DEBUG)
            printf("parse_shapecode: new_init_cond failed!\n");
        return PROJECTM_FAILURE;
    }
    ...
    if (PARSE_DEBUG)
        printf("parse_shapecode: [success]\n");
    return PROJECTM_SUCCESS;
}
...
int Parser::parse_wave_prefix(char *token, int *id, char **eqn_string)
{
    ...
    *eqn_string = token + i + 1;
    ...
    if (PARSE_DEBUG)
        printf("parse_wave_prefix: prefix = %s\n (LINE %d)", *eqn_string, line_count);
    return PROJECTM_SUCCESS;
...
int Parser::parse_wave(char *token, std::istream & fs, MilkdropPreset * preset)
{
    ...
    if (parse_wave_prefix(token, &id, &eqn_type) < 0) {
        if (PARSE_DEBUG)
            printf("parse_wave: syntax error in custom wave prefix!\n");
        return PROJECTM_FAILURE;
    }
...
int Parser::parse_wave_helper(std::istream & fs, MilkdropPreset * preset, int id, char *eqn_type, char *init_string)
{
    ...
    if ((custom_wave = MilkdropPreset::find_custom_object(id, preset->customWaves)) == NULL) {
        if (PARSE_DEBUG)
            printf("parse_wave_helper: custom wave id %d not found!\n", id);
        return PROJECTM_FAILURE;
    }
    ...
    if (!strncmp(eqn_type, WAVE_INIT_STRING, WAVE_INIT_STRING_LENGTH)) {
        ...
        if (PARSE_DEBUG)
            printf("parse_wave_helper (per frame init): [begin] (LINE %d)\n", line_count);

        /* Parse the per frame init equation */
        if ((init_cond = parse_per_frame_init_eqn(fs, preset, &custom_wave->param_tree)) == NULL) {
            if (PARSE_DEBUG)
                printf("parse_wave_helper (per frame init): equation parsing failed (LINE %d)\n", line_count);
            return PROJECTM_PARSE_ERROR;
        }
    ...
    if (!strncmp(eqn_type, PER_FRAME_STRING_NO_UNDERSCORE, PER_FRAME_STRING_NO_UNDERSCORE_LENGTH)) {
        ...
        if (PARSE_DEBUG)
            printf("parse_wave_helper (per_frame): [start] (custom wave id = %d)\n", custom_wave->id);
    ...
    if (!strncmp(eqn_type, PER_POINT_STRING, PER_POINT_STRING_LENGTH)) {
        ...
        if (PARSE_DEBUG)
            printf("parse_wave_helper (per_point): per_pixel equation parsing start...(LINE %d)\n", line_count);
        ...
        if (init_string)
            ...
            if (parseToken(fs, string) != tEq) {        /* parse per pixel operator  name */
                if (PARSE_DEBUG)
                    printf
                        ("parse_wave_helper (per_point): equal operator missing after per pixel operator. Last token = \"%s\"  (LINE %d)\n",
                         string, line_count);
        ...
        if ((gen_expr = parse_gen_expr(fs, NULL, preset)) == NULL) {
            if (PARSE_DEBUG)
                printf("parse_wave_helper (per_point): equation evaluated to null? (LINE %d)\n", line_count);
        ...
        if (PARSE_DEBUG)
            printf("parse_wave_helper (per_point): [finished] (custom wave id = %d)\n", custom_wave->id);
        return PROJECTM_SUCCESS;
    }
...
int Parser::parse_shape(char *token, std::istream & fs, MilkdropPreset * preset)
{
    ...
    if (parse_shape_prefix(token, &id, &eqn_type) < 0) {
        if (PARSE_DEBUG)
            printf("parse_shape: syntax error in custom shape prefix!\n");
        return PROJECTM_PARSE_ERROR;
    }
...
int Parser::parse_shape_per_frame_init_eqn(std::istream & fs, CustomShape * custom_shape, MilkdropPreset * preset)
{
    ...
    if (PARSE_DEBUG)
        printf("parse_shape (per frame init): [begin] (LINE %d)\n", line_count);

    /* Parse the per frame equation */
    if ((init_cond = parse_per_frame_init_eqn(fs, preset, &custom_shape->param_tree)) == NULL) {
        if (PARSE_DEBUG)
            printf("parse_shape (per frame init): equation parsing failed (LINE %d)\n", line_count);
        return PROJECTM_PARSE_ERROR;
    }
