...
void PresetOutputs::PerPixelMath(const PipelineContext & context)
{
    ...
    for (x = 0; x < gx; x++) {
        for (y = 0; y < gy; y++) {
            fZoom2 = powf(this->zoom_mesh[x][y], powf(this->zoomexp_mesh[x][y], rad_mesh[x][y] * 2.0f - 1.0f));
    ...
    for (x = 0; x < gx; x++) {
        for (y = 0; y < gy; y++) {
            this->x_mesh[x][y] += this->warp_mesh[x][y] * 0.0035f * sinf(fWarpTime * 0.333f
                                                                         + fWarpScaleInv * (this->orig_x[x][y] * f[0] -
                                                                                            this->orig_y[x][y] * f[3]));
            this->y_mesh[x][y] +=
                this->warp_mesh[x][y] * 0.0035f * cosf(fWarpTime * 0.375f -
                                                       fWarpScaleInv * (this->orig_x[x][y] * f[2] +
                                                                        this->orig_y[x][y] * f[1]));
            this->x_mesh[x][y] +=
                this->warp_mesh[x][y] * 0.0035f * cosf(fWarpTime * 0.753f -
                                                       fWarpScaleInv * (this->orig_x[x][y] * f[1] -
                                                                        this->orig_y[x][y] * f[2]));
            this->y_mesh[x][y] +=
                this->warp_mesh[x][y] * 0.0035f * sinf(fWarpTime * 0.825f +
                                                       fWarpScaleInv * (this->orig_x[x][y] * f[0] +
                                                                        this->orig_y[x][y] * f[3]));
        }
    }
...
void PresetMerger::MergePresets(PresetOutputs & A, PresetOutputs & B, double ratio, int gx, int gy)
{
    ...
    for (PresetOutputs::cshape_container::iterator pos = A.customShapes.begin(); pos != A.customShapes.end(); ++pos) {
        (*pos)->a *= invratio;
        (*pos)->a2 *= invratio;
        (*pos)->border_a *= invratio;
    }

    for (PresetOutputs::cshape_container::iterator pos = B.customShapes.begin(); pos != B.customShapes.end(); ++pos) {
        (*pos)->a *= ratio;
        (*pos)->a2 *= ratio;
        (*pos)->border_a *= ratio;
    ...
    for (PresetOutputs::cwave_container::iterator pos = A.customWaves.begin(); pos != A.customWaves.end(); ++pos) {
        (*pos)->a *= invratio;
    ...
    for (PresetOutputs::cwave_container::iterator pos = B.customWaves.begin(); pos != B.customWaves.end(); ++pos) {
        (*pos)->a *= ratio;
        for (int x = 0; x < (*pos)->samples; x++) {
