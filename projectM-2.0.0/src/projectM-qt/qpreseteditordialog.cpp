...
QPresetEditorDialog::QPresetEditorDialog(QProjectMWidget * widget, QWidget * parent, Qt::WindowFlags f):QDialog(parent, f),
_qprojectMWidget
(widget)
{
    _ui.setupUi(this);
...
const QString & QPresetEditorDialog::presetUrl() const const
{
    return m_presetUrl;
}
...
void QPresetEditorDialog::saveFile()
{
    ...
    if (!qfile.open(QIODevice::ReadWrite | QIODevice::Text)) {
        QMessageBox::warning(0, "Preset File Error",
                             QString(tr
                                     ("There was a problem trying to save the preset \"%1\".  You may not have permission to write to the file.")).
                             arg(presetUrl()));
        return;
    }
