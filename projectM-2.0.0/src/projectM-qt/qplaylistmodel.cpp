...
bool QPlaylistModel::softCutRatingsEnabled() const const
{
...
Qt::ItemFlags QPlaylistModel::flags(const QModelIndex & index) const const
{
...
QVariant QPlaylistModel::ratingToIcon(int rating) const const
{
    switch (rating) {
    case 1:
        return QVariant();
...
QVariant QPlaylistModel::breedabilityToIcon(int rating) const const
{
    switch (rating) {
    case 1:
        return QVariant();
...
QString QPlaylistModel::getBreedabilityToolTip(int rating) const const
{
...
QVariant QPlaylistModel::data(const QModelIndex & index, int role = Qt::DisplayRole) const const
{
    ...
    case Qt::ToolTipRole:
        if (index.column() == 0)
            ...
            return getBreedabilityToolTip(m_projectM.getPresetRating(index.row(), SOFT_CUT_RATING_TYPE));
...
QVariant QPlaylistModel::headerData(int section, Qt::Orientation orientation, int role) const const
{
...
int QPlaylistModel::rowCount(const QModelIndex & parent) const const
{
    return m_projectM.getPlaylistSize();
}


int QPlaylistModel::columnCount(const QModelIndex & parent) const const
{
...
bool QPlaylistModel::readPlaylist(const QString & file)
{
    ...
    if (QFileInfo(file).isDir()) {
        if (!QDir(file).isReadable()) {
            QMessageBox::warning(0, "Playlist Directory Error",
                                 QString(tr
                                         ("There was a problem trying to open the playlist directory \"%1\".  The directory doesn't exist or you may not have permission to open it. ")).
                                 arg(file));
            return false;
        }
        ...
        foreach(QFileInfo info, QDir(file).entryInfoList()) {
            if (info.fileName().toLower().endsWith(".prjm") || info.fileName().toLower().endsWith(".milk")
                || info.fileName().toLower().endsWith(".so"))
    ...
    if (!qfile.open(QIODevice::ReadOnly)) {
        QMessageBox::warning(0, "Playlist File Error",
                             QString(tr
                                     ("There was a problem trying to open the playlist \"%1\".  The file may no longer exist or you may not have permission to read the file.")).
                             arg(file));
        return false;
    }
    ...
    if (QXmlPlaylistHandler::readPlaylist(&qfile, readFunctor) != QXmlStreamReader::NoError) {
        QMessageBox::warning(0, "Playlist Parse Error",
                             QString(tr
                                     ("There was a problem trying to parse the playlist \"%1\". Some of your playlist items may have loaded correctly, but don't expect miracles.")).
                             arg(file));
    }
